...
import { loadSpace1999Visits, type Space1999Visit } from '../../lib/data/space1999-log'

// /space1999-log - Thriver-only dashboard of visitors to /space1999. That page
// lives in its own repo (github.com/XeroSumGames/space1999generator) and is
// proxied here via a next.config rewrite, so it runs on this origin; its visit
// beacon posts page='/space1999' to log-visit -> visitor_logs, which this reads.
// Mirrors app/ape-log/page.tsx.
...
export default function Space1999LogPage() {
    ...
    <Shell>
      {loading ? <p style={p()}>Loading visits...</p> : (
        <>
          ...
          {visitors.length === 0 ? (
            <p style={{ ...p(), color: C.muted }}>No visits logged yet. Once someone opens the Space: 1999 generator, they will show up here.</p>
          ) : (
            <>
