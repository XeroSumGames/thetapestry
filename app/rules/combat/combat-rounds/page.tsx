...
const COMBAT_ACTIONS: Array<[string, string, string]> = [
  ...
  ['Grapple', '1', 'Opposed Physicality + Unarmed Combat; the defender may resist with Athletics in place of Unarmed Combat. The winner restrains the loser, who takes 1 RP and loses their next Combat Action (whenever that next action falls). While maintaining the grapple, the grappler may spend an action to Subdue the pinned defender: an opposed check (grappler Physicality + Unarmed Combat vs the defender Physicality + Unarmed Combat or Athletics, to see if they are too slippery to hold) that, on the grappler win, deals 1 WP plus (1d3 + Physicality AMod + Unarmed Combat SMod) in RP - the choke that wears them down rather than wounds them. Break Free is the defender spending an action on the same opposed Grapple check; the grappler contests it reactively at no action cost; a failed escape costs the defender 1 RP, and a success simply ends the grapple. A third party who attacks the pinned defender gets +1 CMod to hit (a held target cannot dodge); on a miss, that attacker automatically makes a second attack at -1 CMod against the grappler, risking an unintended hit. Any successful strike against the grappler - regardless of damage dealt - breaks their hold and frees the defender. Release ends the grapple and frees the defender at no cost.'],
