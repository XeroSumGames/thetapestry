...
import { loadWalkingDeadVisits, type WalkingDeadVisit } from '../../lib/data/walkingdead-rpg-log'

// /walkingdead-rpg-log - Thriver-only dashboard of visitors to /walkingdead-rpg.
// That page lives in its own repo (github.com/XeroSumGames/walkingdead-rpg) and
// is proxied here via a next.config rewrite, so it runs on this origin; its
// visit beacon posts page='/walkingdead-rpg' to log-visit -> visitor_logs, which
...
export default function WalkingDeadLogPage() {
  const [authChecked, setAuthChecked] = useState(false)
  const [allowed, setAllowed] = useState(false)
  const [loading, setLoading] = useState(true)
  const [visits, setVisits] = useState<WalkingDeadVisit[]>([])
  ...
  useEffect(() => {
    (async () => {
      ...
      if (ok) { setNowTs(Date.now()); setVisits(await loadWalkingDeadVisits(LIMIT)); setLoading(false) }
    })()
  }, [])
    ...
    <Shell>
      {loading ? <p style={p()}>Loading visits...</p> : (
        <>
          ...
          {visitors.length === 0 ? (
            <p style={{ ...p(), color: C.muted }}>No visits logged yet. Once someone opens the Walking Dead generator, they will show up here.</p>
          ) : (
            <>
...
function Shell({ children }: { children: React.ReactNode }) {
  return (
    <div style={{ maxWidth: '1000px', margin: '0 auto', padding: '28px 20px 80px' }}>
      ...
      <h1 style={{ fontFamily: 'Distemper, Carlito, sans-serif', fontSize: '30px', fontWeight: 700, letterSpacing: '.04em', textTransform: 'uppercase', color: C.ink, margin: '0 0 6px', borderBottom: `1px solid ${C.red}`, paddingBottom: '12px' }}>Walking Dead Generator Log</h1>
      <p style={{ color: C.muted, fontFamily: sans, fontSize: '14.5px', maxWidth: '64ch', margin: '0 0 8px' }}>Everyone who has opened The Walking Dead Universe character generator - where they are, their IP, and how many times they have come back. Visits from the site owner are not logged.</p>
      {children}
    </div>
  )
}
