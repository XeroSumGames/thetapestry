...
export default function SceneControlsPopoutPage() {
  ...
  // Cap at 300. High-DPI maps (e.g. 4800x6600 designed for a 16x22 grid)
  // need cellPx=300 to auto-fit to their intended grid. The DB constraint
  // matches. The recurring 'cellPx mysteriously reaches 200' playtest bug
  // is fixed at the code level (persist only fires on stepper click); the
  // cap is defense in depth, not the primary lock.
  ...
  return (
    <div style={{ background: '#0d0d0d', height: '100vh', padding: '8px', display: 'flex', flexDirection: 'column', gap: '4px', overflow: 'auto', boxSizing: 'border-box' }}>
      ...
      <select value={scene.id} onChange={e => activateScene(e.target.value)}
        style={{ padding: '4px 8px', background: 'rgba(15,15,15,.85)', border: '1px solid #3a3a3a', borderRadius: '3px', color: '#f5f2ee', fontSize: '13px', fontFamily: 'Carlito, sans-serif', textTransform: 'uppercase', cursor: 'pointer', width: '100%', height: '28px', boxSizing: 'border-box', textAlign: 'center', marginBottom: '4px' }}>
      ...
      <div style={{ borderTop: '1px solid #2e2e2e', paddingTop: '6px', marginTop: '4px' }}>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '2px', marginBottom: '4px' }}>
          ...
          <span style={{ fontSize: '13px', color: '#f5f2ee', fontFamily: 'Carlito, sans-serif', minWidth: '34px', textAlign: 'right' }}>{Math.round(gridOpacity * 100)}%</span>
        </div>
      ...
      <button onClick={toggleLock}
        style={{ ...btn, background: mapLocked ? '#2a1210' : 'rgba(15,15,15,.85)', border: `1px solid ${mapLocked ? '#c0392b' : '#3a3a3a'}`, color: mapLocked ? '#f5a89a' : '#f5f2ee' }}>
        {mapLocked ? 'Unlock Map' : 'Lock Map'}
      </button>

      <button onClick={() => setShowRangeOverlay(p => !p)}
        style={{ ...btn, background: showRangeOverlay ? '#1a2e10' : 'rgba(15,15,15,.85)', border: `1px solid ${showRangeOverlay ? '#2d5a1b' : '#3a3a3a'}`, color: showRangeOverlay ? '#7fc458' : '#f5f2ee' }}>
...
const btn: React.CSSProperties = { padding: '4px 10px', background: 'rgba(15,15,15,.85)', border: '1px solid #3a3a3a', borderRadius: '3px', color: '#f5f2ee', fontSize: '13px', fontFamily: 'Carlito, sans-serif', textTransform: 'uppercase', cursor: 'pointer', textAlign: 'center', width: '100%', height: '28px', display: 'flex', alignItems: 'center', justifyContent: 'center', boxSizing: 'border-box' }
const btnSmall: React.CSSProperties = { padding: '4px 6px', background: 'rgba(15,15,15,.85)', border: '1px solid #3a3a3a', borderRadius: '3px', color: '#f5f2ee', fontSize: '13px', fontFamily: 'Carlito, sans-serif', textAlign: 'center', display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: 'pointer' }
const stepBtn: React.CSSProperties = { background: 'none', border: 'none', color: '#f5f2ee', cursor: 'pointer', fontSize: '14px', padding: 0 }
