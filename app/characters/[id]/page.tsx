    ...
    <div style={{ maxWidth: '720px', margin: '0 auto', padding: '1.5rem 1rem 4rem', fontFamily: 'Barlow, sans-serif' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '16px', borderBottom: '1px solid #c0392b', paddingBottom: '12px', marginBottom: '1.5rem' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: '13px', color: '#b0aaa4' }}>
        ...
        <div style={{ display: 'flex', gap: '6px', flexShrink: 0 }}>
          ...
          <button onClick={() => router.push('/characters')}
            style={{ padding: '6px 14px', background: 'none', border: '1px solid #3a3a3a', borderRadius: '3px', color: '#b0aaa4', fontSize: '12px', fontFamily: 'Barlow Condensed, sans-serif', letterSpacing: '.06em', textTransform: 'uppercase', cursor: 'pointer' }}>
            Back
          </button>
        </div>
      </div>
          ...
          {ATTR_KEYS.map(k => {
            ...
            return (
              <div key={k} style={{ background: val > 0 ? '#2a1210' : '#242424', border: `1px solid ${val > 0 ? '#c0392b' : '#3a3a3a'}`, borderRadius: '3px', padding: '8px 4px', textAlign: 'center' }}>
                ...
                <div style={{ fontSize: '8px', color: '#b0aaa4', marginBottom: '4px' }}>{ATTR_FULL[k]}</div>
                <div style={{ fontSize: '18px', fontWeight: 700, fontFamily: 'Barlow Condensed, sans-serif', color: val > 0 ? '#f5a89a' : '#f5f2ee' }}>{sgn(val)}</div>
                <div style={{ fontSize: '8px', color: val > 0 ? '#f5a89a' : '#b0aaa4' }}>{ATTRIBUTE_LABELS[val]}</div>
              </div>
            )
          })}
        </div>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: '6px' }}>
          {[
            ...
            <div key={label} style={{ background: '#242424', border: `1px solid ${hi ? '#7a1f16' : '#2e2e2e'}`, borderRadius: '3px', padding: '8px 10px' }}>
              <div style={{ fontSize: '9px', color: '#b0aaa4', textTransform: 'uppercase', letterSpacing: '.05em', marginBottom: '2px' }}>{label}</div>
              <div style={{ fontSize: '22px', fontWeight: 700, fontFamily: 'Barlow Condensed, sans-serif', color: hi ? '#f5a89a' : '#f5f2ee' }}>{value}</div>
            </div>
          ))}
        </div>
      </div>
      ...
      {skills.length > 0 && (
        <div style={section}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '4px' }}>
            {skills.map((s: any) => (
              <span key={s.skillName} style={{ fontSize: '12px', padding: '3px 10px', borderRadius: '3px', background: '#2a1210', border: '1px solid #7a1f16', color: '#f5a89a', fontFamily: 'Barlow Condensed, sans-serif', letterSpacing: '.04em' }}>
