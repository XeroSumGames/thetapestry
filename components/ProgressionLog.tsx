    ...
    <div>
      ...
      {!compact && log.length > 3 && (
        <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap', marginBottom: '6px' }}>
          ...
          {filter && (
            <button onClick={() => setFilter(null)} style={{ padding: '1px 6px', borderRadius: '2px', fontSize: '13px', fontFamily: 'Carlito, sans-serif', textTransform: 'uppercase', cursor: 'pointer', background: 'none', border: '1px solid #3a3a3a', color: '#f5a89a' }}>Clear</button>
          )}
        </div>
      )}
      ...
      {/* Entries */}
      {displayed.length === 0 ? (
        <div style={{ fontSize: '13px', color: '#cce0f5', fontFamily: 'Carlito, sans-serif', textTransform: 'uppercase', textAlign: 'center', padding: '12px' }}>No entries yet</div>
      ) : (
