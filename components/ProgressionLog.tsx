  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '6px', marginBottom: '6px' }}>
        <span style={{ fontSize: '12px', color: '#c0392b', fontWeight: 700, letterSpacing: '.12em', textTransform: 'uppercase', fontFamily: 'Barlow Condensed, sans-serif' }}>Progression Log</span>
        <span style={{ fontSize: '12px', color: '#5a5550', fontFamily: 'Barlow Condensed, sans-serif' }}>{log.length} entries</span>
        {canEdit && (
          <button onClick={() => setShowAdd(!showAdd)} style={{ marginLeft: 'auto', padding: '2px 8px', background: '#242424', border: '1px solid #3a3a3a', borderRadius: '3px', color: '#d4cfc9', fontSize: '12px', fontFamily: 'Barlow Condensed, sans-serif', textTransform: 'uppercase', cursor: 'pointer' }}>
            {showAdd ? 'Cancel' : '+ Note'}
          </button>
        )}
      </div>
      ...
      {showAdd && (
        <div style={{ marginBottom: '6px', display: 'flex', gap: '4px' }}>
          ...
          <button onClick={addNote} disabled={!noteText.trim()}
            style={{ padding: '4px 8px', background: '#1a2e10', border: '1px solid #2d5a1b', borderRadius: '3px', color: '#7fc458', fontSize: '12px', fontFamily: 'Barlow Condensed, sans-serif', textTransform: 'uppercase', cursor: noteText.trim() ? 'pointer' : 'not-allowed' }}>Add</button>
        </div>
      )}
        ...
        <div style={{ display: 'flex', gap: '3px', flexWrap: 'wrap', marginBottom: '6px' }}>
          {Object.entries(TYPE_LABELS).filter(([type]) => log.some(e => e.type === type)).map(([type, label]) => (
            <button key={type} onClick={() => setFilter(filter === type ? null : type)}
              style={{ padding: '1px 6px', borderRadius: '2px', fontSize: '12px', fontFamily: 'Barlow Condensed, sans-serif', textTransform: 'uppercase', cursor: 'pointer', background: filter === type ? '#242424' : 'transparent', border: `1px solid ${filter === type ? TYPE_COLORS[type] : '#3a3a3a'}`, color: filter === type ? TYPE_COLORS[type] : '#5a5550' }}>
              {label}
            </button>
          ))}
          {filter && (
            <button onClick={() => setFilter(null)} style={{ padding: '1px 6px', borderRadius: '2px', fontSize: '12px', fontFamily: 'Barlow Condensed, sans-serif', textTransform: 'uppercase', cursor: 'pointer', background: 'none', border: '1px solid #3a3a3a', color: '#f5a89a' }}>Clear</button>
          )}
        </div>
      )}
      ...
      {compact && log.length > 10 && (
        <div style={{ fontSize: '12px', color: '#5a5550', fontFamily: 'Barlow Condensed, sans-serif', textAlign: 'center', padding: '4px', textTransform: 'uppercase' }}>
