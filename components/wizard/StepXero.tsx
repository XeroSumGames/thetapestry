    ...
    <div>
      ...
      <div style={{ display: 'flex', alignItems: 'center', gap: '12px', marginBottom: '1rem' }}>
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: '6px' }}>
          ...
          <button onClick={() => setShowPicker(true)} style={nbtn}>Pick from library</button>
      ...
      {showPicker && (
        <PortraitBankPicker
          initialGender={
            /female|woman/i.test(state.gender) ? 'woman'
              : /male|man|^m$/i.test(state.gender) ? 'man'
              : 'all'
          }
            ...
            <div style={{ position: 'relative' }}>
              ...
              {state.threeWords[i] && (
                <button onClick={() => clearWord(i)}
                  style={{ position: 'absolute', right: '8px', top: '50%', transform: 'translateY(-50%)', background: 'none', border: 'none', color: '#f5f2ee', cursor: 'pointer', fontSize: '14px', lineHeight: 1, padding: 0 }}>
                  x
                </button>
              )}
            </div>
