...
- **The `lib/data` seam is CLOSED, not merely ratcheted - a new DB read needs a `lib/data` home from the start.** `scripts/check-arch.mjs` counts `.from(` outside `lib/data` as `seamLeakage.fromOutsideData`, the baseline sits at 923, and the current count IS 923: zero headroom. Unlike the LOC ceilings, which get a 25-line grace band, seam leakage is compared strictly, so ONE new inline `.from(` in a component or page fails the pre-commit hook outright. Write the query as an exported builder in `lib/data/*` when you write it; retrofitting it after the hook rejects your commit is the expensive order. (Hit 2026-09-16 building the Campaign Sheet NPCs Met panel.)
...
- **No em-dashes / en-dashes in live function or view bodies.** The no-em-dash rule (memory `feedback_no_en_dash`) applies to text DB functions/triggers emit to users (e.g. the community-growth notification). The pre-commit `check-em-dashes.mjs` can't see that text (it lives in `pg_proc` bodies, and the only repo mirror `sql/_baseline/schema.sql` is exempt as a faithful capture). **Guardrail:** `npm run check:db-emdashes` (`scripts/check-db-emdashes.mjs`) queries live function/view bodies for em/en-dashes. Same posture as `check:publication` - needs the linked DB, ON-DEMAND / pre-ship, skips loudly when offline. To fix an offender: `CREATE OR REPLACE` with the exact live body (from `pg_get_functiondef`) and only the dash swapped (reference `sql/fix-db-function-emdashes-2026-05-24.sql`).
