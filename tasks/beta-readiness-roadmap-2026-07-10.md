...
- [ ] **Combat damage uses the WRONG character's Physicality.** `useRollResolution.ts:365` (+dup :1907) reads `myEntry` (the viewer) instead of the attacker, so every GM-run NPC attack and every GM-rolled PC attack deals wrong damage. **Same root fix also cures the next item** - do them together.
- [ ] **NPCs have infinite ammo + NPC shots drain the GM's own PC's ammo.** `useRollResolution.ts:591` decrements the viewer's weapon slots, not the attacker's. Same `myEntry` root cause; also mis-targets the jam path (:1202) and kill-credit (:755). The 2026-06-01 "Cree fix" patched only the Insight half - finish the job on the main path.
