...
## Current HEAD: derive (`git rev-parse --short HEAD`; was `4b528f9` after the MapView seam migration). page.tsx 10553; 548 tests; CI green (full fitness set). Ratchet baselines current (`.from` 1182, `.channel` 32, console 115).
...
## Phase 5 IN PROGRESS - 2 of 6 god-components done; vehicle next
...
- **NEXT: `app/vehicle/page.tsx`** (2111 LOC, 19 `.from`, 6 channels + 1 bcast). Has BOTH seams - reuse `lib/data/map.ts`-style repo for the data side and BOTH realtime primitives (useCampaignChannel for any campaign-scoped + broadcast channel; usePostgresSubscription for any global single-table watch). Then `components/NpcRoster.tsx` (2302) -> `components/CampaignCommunity.tsx` (3159) -> `components/TacticalMap.tsx` (4315, hardest).
