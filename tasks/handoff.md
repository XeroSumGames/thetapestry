...
## Current main HEAD
Derive it: `git rev-parse --short HEAD` (was `74e137c` at handoff write; page.tsx 12557 lines / check-arch ceiling 12558; 513 vitest green). Verify line count + ratchets with `node scripts/check-arch.mjs` and `wc -l "app/stories/[id]/table/page.tsx"`.

## Where we are (Phase 3, hook phase)
- DONE: Phase 1 (fitness harness `scripts/check-arch.mjs` + `lib/realtime/` seam + `lib/data/` seam), Phase 2 gating (initiative-actions tested), the mechanical `.from` migration (seam-leakage 1324 -> 1266, roll_log fully migrated repo-wide, campaign_npcs reads, 5 GM modals seam-consistent), AND **`useGmTools` Part A complete (2026-05-22, commits `3bc2bf2`..`85c9458`)** - all 9 GM-modal state clusters (restore/reload/loot/cdp/populate/advanceTime/endSession/community+openCommunityModal/quickAdd) lifted out of page.tsx into `app/stories/[id]/table/hooks/useGmTools.ts`, one cluster per commit, page destructures so every call site is unchanged, behavior-identical + tsc-verified. Page LOC ratchet 12566 -> 12558.
- NEXT (fresh context, in order): the four combat-critical hooks - `useTacticalSync` (3a, fixes GM-pinned-to-tactical; needs the GM-can-preview-while-sharing design confirm, assume yes) -> `useInitiative` (3b, fixes nextTurn perf) -> `useRollResolution` (3c, fixes CMod-dropped; rebuilds ~1,850-line executeRoll) -> `useTableRealtime`/`useCampaignState`/`useTableAuth` (3d, onto useCampaignChannel). Each fixes its smoke-found bug as **fix-do-NOT-preserve** (list: grand plan Phase 3 + tasks/todo.md lines ~28/30/32). 3c + 3d need Xero's **2-client smoke** as the acceptance gate. **Do these with fresh context - a silent combat-math slip is the one outcome worse than slow progress.**
