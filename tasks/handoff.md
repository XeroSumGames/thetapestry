...
## What shipped this session (2026-06-12, Puffer lane - two sub-sessions)

**Sub-session 1 (Advantages + FI doc):**
- **`0f8e893`** feat(advantages): Use button opens matching skill roll modal. `handleUseAdvantage` -> `pendingAdvantage` + `handleRollRequest`; on `didRoll=true` fires `consumeAdvantage` + feed broadcast; cancel = no consume. Live-verified: full grant->use->consume loop clean.
- **`4be39cf`** docs(puffer): HOPED-FOR rewritten - Advantages drained to PLAYTESTED; FI Insight Die EARN vs SPEND clarified.

**Sub-session 2 (stability audit + infra fixes):**
- **`0f2cc28`** docs(stability-audit): full pre-Beta-500 audit `tasks/stability-audit-2026-06-12.md`. All gates green; 0 BLOCKER; 0 HIGH; 2 MEDIUM (schema baseline drift + em-dashes); 4 LOW. roll_log YELLOW -> GREEN-ish; TacticalMap 14-day watch CLOSED; Confidence Ledger refreshed 875/48. debug-handoff.md + todo.md + security-audit.md updated.
- **`d753be9`** fix(baseline): sync `sql/_baseline/schema.sql` - 6 drifts: `portrait_bank` (nullable number/gender, +is_private, +name, dropped gender_check constraint) + `initiative_order.pending_action_loss`. Text edits only, no DB action. Closes AUDIT M1.
- **`e75d501`** fix(sql): remove em-dashes from `loot_npc_item` pg_proc body via CREATE OR REPLACE (`sql/fix-loot-npc-item-emdashes-2026-06-12.sql`). check:db-emdashes now CLEAN. Closes AUDIT M2.
- **`14f5939`** docs(todo): AUDIT M1 + M2 marked done.

H&P also shipped between last Puffer session and now:
...
## STILL HOPED-FOR (drain target: Beta-500 7/1 dry-run)

1. **FI Insight Die EARN path** - `useRollResolution.ts:264` `insight_dice +1` never fired in live play. Trigger: any 2d6 roll hits 1+1 or 6+6. No code owed; needs a live session where doubles come up.
2. **Stress Check 12-string narrative** - HEAL/UNJAM/REPAIR/Gut Instinct/Group Check/DRIVE/BREW/NAVIGATE not yet captured. Drain target: Beta-500 full dry-run.

## OPEN Puffer queue

1. **`check-realtime-wrap.mjs` regression guardrail** (todo.md FOLLOW-UP) - Path B closed the gap. Guardrail can now be added to pre-commit. Lint: `supabase\.channel\(` outside `lib/realtime/*` + allowlist (send-only: GmNotes:494, campaign-clock.ts x3; presence: table page :1316, Sidebar:85). Low urgency.
2. **Grapple Phase 4** - modal polish + `executeRoll` consolidation onto `applyDamageToPc/Npc`. Phase 3 column live. H&P owns; Puffer tracks.
