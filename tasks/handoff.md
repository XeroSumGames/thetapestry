...
## Current HEAD: derive (`git rev-parse --short HEAD`; was `91e85fc` after vehicle realtime seam). page.tsx 10553; 548 tests; CI green (full fitness set). Ratchet baselines current (`.from` 1168, `.channel` 26, console 115).
...
## Phase 5 IN PROGRESS - 3 of 6 god-components done; NpcRoster next
...
- **NEXT: `components/NpcRoster.tsx`** (2302 LOC, ~50 `.from`, 1 ch + 6 pg). Big data seam (reuse the map/vehicle repo pattern) + realtime (useCampaignChannel for the broadcast/postgres channel, or usePostgresSubscription if global). Then `components/CampaignCommunity.tsx` (3159) -> `components/TacticalMap.tsx` (4315, hardest). The realtime toolkit is now complete: useCampaignChannel (campaign broadcast+postgres), usePostgresSubscription (global single-table), broadcastOnce (fire-and-forget send).
