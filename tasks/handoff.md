...
## Current HEAD: derive (`git rev-parse --short HEAD`; was `d92aba5` after NpcRoster seams). page.tsx 10553; 548 tests; CI green (full fitness set). Ratchet baselines current (`.from` 1120, `.channel` 25, console 115).
...
## Phase 5 IN PROGRESS - 4 of 6 god-components done; CampaignCommunity next
...
- **NEXT: `components/CampaignCommunity.tsx`** (3158 LOC, 56 `.from`, 1 ch + 1 pg). **RECON (2026-05-23, verified by grep):** 13 tables - community_members (heaviest), communities, world_communities, community_stockpile_items, campaign_members, campaign_pins, campaign_npcs, community_morale_checks, community_events, community_migrations, characters, profiles, roll_log. **REUSE `lib/data/roll-log.ts` insertRollLog** for the roll_log site (~:563); communities/campaign_npcs/campaign_members likely overlap existing repos (check campaign-npcs.ts, moderation.ts before duplicating). Put community-specific shapes in a NEW `lib/data/community.ts`. 3 `console.*` -> Phase 6. The 1 channel + 1 pg is low realtime risk (likely a community_members/communities refetch sub -> useCampaignChannel). Do data seam first (safe, big), then the 1 channel - SAME split as the others. Started fresh-window per the "big god-components need fresh focus" lesson (this run shipped 4/6; CampaignCommunity + TacticalMap are the two biggest left).
- **THEN: `components/TacticalMap.tsx`** (4315, hardest - 2 ch / 7 bcast / 6 pg; the combat-critical tactical_ channel + token broadcasts - careful, like vehicle; uses the same `token_moved`/`firing_arc_toggle`/`vehicle_updated` events already in events.ts). The realtime toolkit is complete: useCampaignChannel (campaign broadcast+postgres, incl. `postgres[]` arrays), usePostgresSubscription (global single-table), broadcastOnce (fire-and-forget send). Per-component: data seam first (safe), then realtime; reference checklist in architecture-target.md; technique in lessons.md top 4 entries.
