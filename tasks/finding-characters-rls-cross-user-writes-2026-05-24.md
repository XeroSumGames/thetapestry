...
**Status:** **GM-half FIXED + verified live 2026-05-24** (Xero applied `sql/characters-gm-write-rls-2026-05-24.sql`; the "GM can update characters in their campaigns" UPDATE policy is present on `characters`). Flows 2-8 resolved. **Flow 1 (PC-to-PC trade): DECISION = Option B (the RPC), Xero 2026-05-24.** RPC **APPLIED + verified live 2026-05-24** (`give_item_to_character`, SECURITY DEFINER, present on prod). BUT nothing calls it yet - the client `onGiveItem` still does the old raw `.update`, so trade still loses data. **SOLE remaining blocker: Hunt & Peck rewires `onGiveItem`** to call the RPC + drop both raw inventory writes (contract in the SQL footer); then E2E un-fixmes the trade assertion. Risk Register stays YELLOW until the client rewire lands; then GREEN.
