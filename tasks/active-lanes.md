...
## Playwright / E2E
- **focus:** Phase 2: 4 of 7 SHIPPED (#8 char-create, #9 session-lifecycle, #14 rumors-publish-clone, #11 npc-roster-crud) + map_pins bypass-regression - all green standalone, ZERO app-code edits. A full re-cert caught a real insert-ordering RACE in #14 (module row commits before its version row; one-shot query lost under load) -> FIXED (poll the version). NOW building #13 `inventory-trade` (Sys J): the add-item/encumbrance/persist half is clean+reversible; the PC<->PC trade half is a CONFIRMED PROD DATA-LOSS BUG (Survivor giver can't write the receiver's characters row under RLS, item destroyed) -> ROUTED to Puffer Fish (RLS/RPC) + Hunt & Peck (client), see tasks/finding-pc-trade-rls-dataloss-2026-05-24.md; the trade assertion ships as test.fixme pending the RPC. REMAINING after #13: communities-lifecycle (#12), combat-flow (#10, last). Testid policy "A".
- **touching:** `e2e/` (additive specs only - no app-code edits). New finding doc `tasks/finding-pc-trade-rls-dataloss-2026-05-24.md`. Docs: `tasks/todo.md`, `tasks/beginners-guide-testplan.md`, `tasks/lessons.md`, `tasks/e2e-results-2026-05-24.html`.
- **updated:** 2026-05-24, working from HEAD 616cb2e (lane/e2e; rebases on push).
