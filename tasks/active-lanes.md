...
## Playwright / E2E
- **focus:** Phase 2: 4 of 7 SHIPPED this session (all green on prod, ZERO app-code edits). FULL-SUITE CERTIFIED (117+2-flaky): #8 `char-create-methods` (Ch4.2-4.4), #9 `session-lifecycle` (Ch7.1p/7.3), + the map_pins bypass-regression. Green standalone (in next re-cert): #14 `rumors-publish-clone` (Ch14.2-3) + #11 `npc-roster-crud` (Ch10.1 create/edit/clone/delete via existing title= controls). REMAINING (3): communities-lifecycle (#12), inventory-trade (#13), combat-flow (#10 - heaviest, app-testid + dice-gated, do LAST + coordinate table-page testids with Hunt & Peck). Testid policy "A": REST/DOM preferred; unavoidable testids = own minimal commit, flagged here first.
...
- **updated:** 2026-05-24, working from HEAD 4f13b29 (lane/e2e; rebases on push).
