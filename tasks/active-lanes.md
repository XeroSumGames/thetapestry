...
## Playwright / E2E
- **focus:** Phase 2 IN PROGRESS. SHIPPED + FULL-SUITE CERTIFIED (re-cert: 117 passed + 2 known realtime-flaky-passed-on-retry, exit 0): #8 `char-create-methods` (Ch4.2-4.4), #9 `session-lifecycle` (Ch7.1 partial/7.3, own throwaway campaign), + the map_pins bypass-regression in `world-pin-to-queue` (Puffer's trigger now has its E2E net). NOW BUILDING: #14 `rumors-publish-clone` (Ch14 - publish PRIVATE module from a throwaway story -> clone -> content lands; pure DOM+REST, no app testids). Remaining after: combat-flow (#10, app-testid + dice-gated, last), npc-roster-crud, communities-lifecycle, inventory-trade. Xero confirmed testid policy "A": REST/DOM assertions preferred; unavoidable testids ship as own minimal commit, flagged here first.
- **touching:** `e2e/` (additive specs only - no app-code edits). `e2e/rumors-publish-clone.spec.ts` (new, in progress). Docs: `tasks/todo.md`, `tasks/beginners-guide-testplan.md`, `tasks/lessons.md`, `tasks/e2e-results-2026-05-24.html`.
- **updated:** 2026-05-24, working from HEAD 018a952 (lane/e2e; rebases on push).
