...
8. **[PF/OP] Observability: "would we know if it broke?"** Sentry (client/server/edge) + `/api/health` are wired; the health-pulse is internal-only. **UPTIME MONITOR DONE 2026-05-29** - Better Stack hits `/api/health` every 30s and pages `#all-xero-sum-games` Slack + email (Slack mobile push = free pager); verified end-to-end. Setup: `tasks/ops-uptime-monitor-setup-2026-05-29.md`. STILL OPEN: confirm Sentry alerts route to a human (the ~5-min alert-rule click-through, todo observability B). At 500 users you want a pager, not a player, to tell you prod is down - that half is now in place.
