...
**T2-1 [HIGH] Campaign-scope the 3 unfiltered table-page subscriptions. ⚠️ DO NOT just add `filter: campaign_id=eq.${id}` - CORRECTED 2026-06-23 (PF).** The fan-out is real (every change to these tables in ANY campaign wakes EVERY table client), BUT none of the 3 tables actually have a `campaign_id` column - verified live:
- `scene_tokens` (~467-468) -> has `scene_id` (FK tactical_scenes), no campaign_id.
- `npc_relationships` (~1594) -> has `npc_id`/`character_id`, no campaign_id.
- `community_members` (~1589) -> has `community_id`, no campaign_id.
A `campaign_id` filter on any of them filters a nonexistent column = a SILENTLY DEAD sub (no events, no error) = realtime BREAKS (AGENTS.md silent-dead-sub class). postgres_changes filters can't join, so you can't reach campaign_id through the FK either.
**Correct fix (PF schema work, then a 1-line filter): denormalize a `campaign_id` column onto each table** (backfill from scene/npc/community -> campaign, maintain via a BEFORE INSERT trigger), THEN `filter: campaign_id=eq.${id}`. Pragmatic option: do it for `scene_tokens` only (the high-volume one - every token drag) and accept the fan-out on `npc_relationships` + `community_members` (low event volume, cheap handlers). This needs a 2-client verify (token move in campaign A does NOT wake campaign B's client, AND still updates A's). **Routed back to PF** - do not ship a raw campaign_id filter.
