...
**Audit:** npm audit [2 high, 0 critical] — unchanged
- HIGH `ws` — memory exhaustion DoS; fix available (`npm audit fix`, non-breaking)
- HIGH `vite` — fs.deny bypass + NTLMv2 leak (Windows/dev); fix available
...
- *HOPED-FOR* FI Insight Die AWARD path — needs live doubles roll to drain
- *Stale-as-open* `scripts/check-realtime-wrap.mjs` — file does not exist; todo item unchecked
- *Stale-as-open* recorder `window.prompt` — still in `PlaytestRecorder.tsx:244`; todo unchecked

**Action:** No new signal. `npm audit fix` (ws+vite) still the open action — HP lane.
...
**CI:** last 5 runs all success (latest 2026-06-17T09:10 UTC)
...
**Action:** No new signal. `npm audit fix` (ws+vite) still owed — HP lane.

---

## 2026-06-17 09:09 UTC

**Status:** RED+DRIFT (carry-forward; no new commits since 06:09 UTC)

**Gates:** font-sizes [OK], role-literals [OK], tsc [OK], tests [875 passed / 48 files]

**Audit:** npm audit [2 high, 0 critical] — unchanged
- HIGH `ws` 8.20.1 CVSS 7.5 — memory exhaustion DoS; fix: `npm audit fix` (bumps >=8.21.0, non-breaking)
- HIGH `vite` 8.0.13 — fs.deny bypass + NTLMv2 leak (Windows / dev-only; fix available)

**CI:** last 5 runs all success (latest 2026-06-17T06:12 UTC)

**Drift:**
- *HOPED-FOR* Vehicle popout broadcasts (Section B) — 25+ days HOPED-FOR; no vehicle commits in 3+ days
- *HOPED-FOR* Stress Check 12-string (HEAL/UNJAM/REPAIR/GI/Group Check/DRIVE/BREW/NAVIGATE) — drain: Beta-500 dry-run
- *HOPED-FOR* FI Insight Die AWARD path — needs live doubles roll to drain
- *Stale-as-open* `scripts/check-realtime-wrap.mjs` — file does not exist; todo item unchecked
- *Stale-as-open* "6 mechanics still owe real code" — all 6 sub-items appear [x]; parent unchecked

**Action:** No new signal. `npm audit fix` (ws+vite, non-breaking) still owed — assign to HP lane.

---

## 2026-06-17 06:09 UTC

**Status:** RED+DRIFT (carry-forward; no new signal since 00:11 UTC)
...
**CI:** last 5 runs all success (latest 2026-06-17T00:12 UTC)
...
**Audit:** npm audit [2 high, 0 critical] — unchanged
- HIGH `ws` CVSS 7.5 — memory exhaustion DoS; fix: `npm audit fix` (bump ws >=8.21.0)
- HIGH `vite` — fs.deny bypass + NTLMv2 leak (dev-only; fix available)

**CI:** last 5 runs all success (latest: 2026-06-16T18:09 UTC, confirmed via GitHub MCP)

**Drift:**
- *HOPED-FOR* Vehicle popout broadcasts (Section B) — no vehicle commits in 3+ days; 23+ days HOPED-FOR
- *HOPED-FOR* Stress Check 12-string (HEAL/UNJAM/REPAIR/GI/Group Check/DRIVE/BREW/NAVIGATE) — drain: Beta-500 dry-run
- *HOPED-FOR* FI Insight Die AWARD path — needs live doubles roll to drain
- *Stale-as-open* `scripts/check-realtime-wrap.mjs` — file still does not exist
- *Stale-as-open* "6 mechanics still owe real code" — all 6 sub-items [x]; parent unchecked

**Action:** No new signal. Same as 18:08 entry: `npm audit fix` (ws+vite, non-breaking, HP lane) still owed.
...
- HIGH `vite` (via vitest): Windows NTLMv2 hash + path bypass — dev-only
- HIGH `ws` (via @supabase/realtime-js): memory exhaustion DoS on ws SERVER — app is client-only

**CI:** run #1035 (2026-06-16T09:10:32Z) conclusion=success — all-pass confirmed via GitHub MCP
...
- *HOPED-FOR* FI Insight Die AWARD path — needs live doubles roll to drain
- *Stale-as-open* `scripts/check-realtime-wrap.mjs` — file still does not exist
- *Stale-as-open* "6 mechanics still owe real code" — all 6 sub-items [x]; parent checkbox not ticked

**Action:** No new signal vs 12:09 entry. `npm audit fix` for vite+ws still owed (HP lane).
...
**CI:** gh CLI absent in sandbox — skipped

**Drift:**
- HOPED-FOR ~22 days: Vehicle popout broadcasts (Section B). No code change since last run.
- HOPED-FOR ~28 days: Stress Check 12-string (HEAL/UNJAM/REPAIR/Gut Instinct/Group Check/DRIVE/BREW/NAVIGATE). Drain target: Beta-500 dry-run ≤2026-07-01.
- HOPED-FOR >3 days: FI Insight Die AWARD path. 3-day flag crossed at 06:08 run; still no code change.
- `debug-handoff.md §3` HOPED-FOR still lists combat-flow Phase B (shipped GREEN 2026-06-11) — flagged 8 consecutive runs, action not taken.
- `scripts/check-realtime-wrap.mjs` absent (todo open ~2026-05-24).
- Stale todos (prev confirmed + newly found): pin realtime catch-up; img_scale eyeball; NEW — "6 mechanics still owe real code for 9/1 KS" (todo.md:67) parent `[ ]` open while all 6 child items are [x] closed.

**Action:** ⚠️ 8th flag — Puffer Fish: drain combat-flow Phase B from debug-handoff §3; tick "6 mechanics" parent todo; close pin + img_scale stale todos; restate or close vehicle popout caveat.
...
**CI:** last 5 runs all success (latest 2026-06-13T06:10Z via GitHub MCP)

**New since 06:15:** `88f90d7 feat(modules): add play_time field with clock chip on module cards` — gates clean, no issues.

**Drift (unchanged from 06:15):**
- HOPED-FOR >14 days (drain target Beta-500 ≤2026-07-01): FI Insight Die AWARD path; Stress Check 12-string (HEAL/UNJAM/REPAIR/Gut Instinct/Group Check/DRIVE/BREW/NAVIGATE); Vehicle popout broadcasts (Section B, 20 days); Combat-flow Phase B (15 days, no `e2e/combat-flow.spec.ts` changes).
- `scripts/check-realtime-wrap.mjs` still absent (todo says "now unblocked").
- Action from 00:11 entry still pending: two stale todos need closing — `img_scale FIXED awaiting 2-client visual eyeball` (tactical map GREEN 2026-05-30; close it) + `[PUFFER - pin realtime propagation]` (catch-up shipped 2026-05-27; close it).

**Action:** Close the two stale todos from 00:11. Schedule Beta-500 dry-run to drain HOPED-FOR items before 7/1.
...
**Gates:** font-sizes [OK], role-literals [OK — 332 files], tsc [OK], tests [875 passed / 48 files]

**Audit:** npm audit [clean — 0 high, 0 critical]

**CI:** last 5 runs all success (latest 2026-06-12T18:06Z)

**Drift (all carry-over; identical to 15:10 UTC run):**
- HOPED-FOR >28d: Tier-2 Recruit, P3 Q4-b Advantages, FI streamline (Insight Die + single-modal), Stress Check 12-string narrative polish. Drain target: Beta-500 dry-run 2026-07-01 (19 days out).
- Confidence Ledger stale: debug-handoff.md §3 says 738 tests/41 files; actual 875/48. Run `npx tsx scripts/refresh-ledger.mjs`.
- `scripts/check-realtime-wrap.mjs` still absent; todo.md says "now unblocked."
- `characters` cross-user parent todo still `- [ ]`; all sub-items `[x]`; Risk Register GREEN/CLOSED 2026-05-25.

**New since 15:10:** 8 commits landed — token-creator folder-picker + privacy toggle, PC portrait = map token (Phase 1), tier-colored silhouette portraits, portrait ring visibility. All gates pass.

**Action:** Carry-over only. Tick closed todos; run ledger refresh; plan Beta-500 dry-run before 7/1.
...
**Gates:** font-sizes [OK], role-literals [OK — 331 files], tsc [OK], tests [873 passed / 48 files] (+8 tests since run 72)

**Audit:** npm audit [clean — 0 high, 0 critical]

**CI:** last 5 runs all success (latest 2026-06-11T20:46Z)

**Drift:**
- HOPED-FOR >23d (carry-over): Tier-2 Recruit (all phases), P3 Q4-b Advantages, FI streamline (Insight Die spend + single-modal), Stress Check 12-string narrative lock. Drain target: Beta-500 dry-run before 2026-07-01 (20 days). Plan exists at `tasks/beta-500-dry-run-testplan.md`.
- **Stale-open (carry-over from run 72 — shipped, still `- [ ]`):** AUDIT L1 resolved `ade341e`, AUDIT L2 closed `2545c89`, Observ-B closed `622c314`. All three still `[ ]` in todo.md lines 66, 67, 117.

**Action:** Tick 3 stale-open boxes: todo.md L66 (AUDIT L1), L67 (AUDIT L2), L117 (Observ-B). Schedule Beta-500 dry-run to drain 4 HOPED-FOR items.

---

## 2026-06-11 18:10 UTC

**Status:** DRIFT (run 72 — 3 stale-open todos; 4 HOPED-FOR carry-over; gates/CI all green)

**Gates:** font-sizes [OK], role-literals [OK — 330 files], tsc [OK], tests [865 passed / 47 files]

**Audit:** npm audit [clean — 0 high, 0 critical]

**CI:** last 5 runs all success (latest 2026-06-11T16:10Z)

**Drift:**
- HOPED-FOR >23d (carry-over): Tier-2 Recruit (all phases), P3 Q4-b Advantages, FI streamline (Insight Die spend + single-modal), Stress Check 12-string narrative lock. 0 commits to these areas since 2026-05-19. Drain target: Beta-500 dry-run before 2026-07-01 (20 days).
- **Stale-open (shipped today, still `- [ ]`):** AUDIT L1 resolved via `ade341e` ("parallelize wound-infection drain"), AUDIT L2 closed via `2545c89` ("all empty .catch sites intentional"). Both shipped 2026-06-11 morning; todo.md not updated.
- **Stale-open (carry-over from run 71):** `[OP] observability B` — closed via `622c314` ("Sentry alert rule live"), still `- [ ]` in todo.md.

**Action:** Tick 3 stale-open boxes in todo.md: AUDIT L1 (line 66), AUDIT L2 (line 67), Observability B (line 117). Then schedule Beta-500 dry-run — only path to drain 4 HOPED-FOR items before 7/1.
...
**CI:** last 5 runs all success (latest 2026-06-11T09:11Z)

**Drift:**
- HOPED-FOR >12d (carry-over): Tier-2 Recruit (all phases), P3 Q4-b Advantages, FI streamline (Insight Die spend + single-modal), Stress Check 12-string narrative lock. 0 commits to these areas. Drain target: Beta-500 dry-run before 2026-07-01.
- `combat-flow Phase A "Start Combat"` E2E regression still open (routed HP 2026-06-01, 10d). Recent combat commits (Subdue, Break Free, Insight Dice, strike-the-grappler, chat ping) did not touch this.
- **NEW stale-todo:** `todo.md` Conditions Phase-2 item (`[VERIFY-FIRST FINDING 2026-05-31]`) self-describes as "Pickup #4 marked DONE" but checkbox is still `- [ ]`. Audit-correction: tick the box or remove the entry.

**Action:** HP: tick/remove the Conditions Phase-2 done-item. Then: schedule Beta-500 dry-run (19 days to 7/1) — only a playtest drains the 4 HOPED-FOR items.
...
**CI:** last 5 runs all success (latest 2026-06-10T21:11Z)

**Drift:**
- HOPED-FOR >23d (carry-over): Tier-2 Recruit (all phases), P3 Q4-b Advantages, FI streamline (Insight Die spend + single-modal), Stress Check 12-string narrative lock. 0 commits to these areas since run 66. Drain target: Beta-500 dry-run before 2026-07-01.
- `combat-flow Phase A "Start Combat"` E2E regression still open (routed to HP 2026-06-01, 10d). No new commits since run 66.

**Action:** same as run 66 — schedule the Beta-500 dry-run; 20 days to 7/1. The 4 HOPED-FOR items can only drain at the table.
