...
**STATUS: COMPLETE 2026-05-22.** 1c `scripts/check-arch.mjs` ratchets (LOC ceilings / seam-leakage 1324+44 / prod-console 118) on pre-commit+CI = `c56def7`. 1a `lib/realtime/` (events registry + useCampaignChannel, `[campaignId]`-deps, sentry-folded) = `d009bf6`. 1b `lib/data/` (db() locally typed as `SupabaseClient<Database>` + campaign-npcs/character-states exemplar repos) = `e24b29c`. All tsc-clean, 502 tests, ratchets at baseline. Nothing consumes the seams yet (that's Phase 3).
...
- **STATUS / JUDGMENT 2026-05-22:** The TRUNK's pure deps (roll math, damage, outcomes, stabilize, range, table-roll-context) were already tested (Step 1), and `initiative-actions` is now tested too (`8a4a371`, 11 tests, 502->513) - so the safety net that actually gates the Phase 3 trunk rebuild is in place. The remaining untested pure modules are mostly static-content/constant files (xse-schema, setting-*, weapons, animals) with little logic, or non-trunk feature logic (module-diff, snapshot-to-module) - test those opportunistically, not as a blocker. The bulk L2 DECOUPLING (moving `.from` out of the 16 data-coupled lib modules incl. modules.ts) is **folded into the per-component migrations (Phase 3/5), ratchet-guided** - decouple a module when the god-component that uses it migrates, rather than a risky big-bang refactor of live features up front. The seam-leakage ratchet enforces the monotonic decrease either way. Net: Phase 2's gating goal is met; proceed to Phase 3.

### PHASE 3 - Migrate the table page fully onto the seams (it becomes THE reference implementation)
**STATUS 2026-05-22 - mechanical .from migration done; hook rebuilds remain.** Behavior-preserving repo migration shipped: `campaign_npcs` full-selects (page + RestorePicker + page.tsx sites), ALL `roll_log` inserts/deletes repo-wide (page + 7 components -> insertRollLog/deleteRollLog), the 5 extracted GM modals, + the campaign-npcs/character-states repos. **Seam-leakage ratchet 1324 -> 1266 (-58), locked.** Lesson: clean mechanical wins = UNIFORM operations (one query shape across many sites) -> named repo fn + string-replace. The remaining ~1266 are (a) small 1-4 site clusters and (b) VARIED writes concentrated in the trunk tables (initiative_order/character_states/scene_tokens) - migrating those now then rebuilding them into the hooks is double-handling, so they migrate AS part of 3a-3d below (useTacticalSync/useInitiative/useRollResolution/useTableRealtime own their tables' queries). The mechanical phase that can run WITHOUT the hooks is complete.
...
- **3c.** Extract trunk hooks on the seams: `useCampaignState`, `useTacticalSync`, `useInitiative` (ReadyWeapon rides here), `useGmTools` (absorbs the modal prop-threading + repays the loose-`any` debt), `useRollResolution` (on the tested roll core), `useTableRealtime`.
  ...
  - **PERF target (useInitiative):** `nextTurn` runs sequential DB round-trips (deactivate -> activate -> reload), measured 656ms-3,638ms/turn in the 2026-05-22 smoke. Batch into one wave + optimistic local turn-flip so it changes instantly (same pattern as the Restore flow).
  - **FIX-DO-NOT-PRESERVE (2) - useTableRealtime/useTacticalSync:** while tactical is SHARED, the GM cannot return to the Campaign map - `scene_activated` (`page.tsx:1351`) + `tactical_shared` (`:1340`) force `setShowTacticalMap(true)` whenever shared, overriding the GM's view toggle (`:7276`). Decouple the GM's local view intent from the shared-state force (sharing drives what PLAYERS see, not the GM's pane). Needs a one-line design confirm from Xero (GM-can-preview-campaign-while-sharing = yes).
