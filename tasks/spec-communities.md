...
- ✅ **§2a Apprentice creation flow** — full 5-step wizard ([`components/ApprenticeCreationWizard.tsx`](../components/ApprenticeCreationWizard.tsx)). **2026-04-30 rewrite:** flipped from Paradigm-based to **Profession-based** per Xero's clarification on the SRD reading.
  - Identity step: name + age + three trait words + freeform background. Age + 3 words auto-rolled at recruit time alongside Motivation/Complication and stored in `community_members.apprentice_meta`; pre-filled but editable.
  - Profession step: pick from the 12 PROFESSIONS (Table 8) via the new [`components/ProfessionPicker.tsx`](../components/ProfessionPicker.tsx). Profession seeds 1 CDP into each of its 5 skills (vocational -3 → -1, non-vocational 0 → +1). Profession does NOT seed RAPID.
  - RAPID step: 3 CDP, baseline 0 across the board, capped at +4 per attribute.
  - Skills step: 5 CDP on top of the profession baseline. Per-skill SRD cap = `min(SKILL_MAX, master_PC.skill − 1)`. Skills the master PC doesn't have at level ≥1 render "untrainable" — profession baseline still stands but no further spend allowed.
  - Confirm + save. Updates `campaign_npcs` (RAPID + skills + structured note block) and bumps `apprentice_meta.setup_complete=true` with `profession`, `age`, `three_words`, `background`. Master PC's progression log gets a memorable-moment entry.
  - Future CDP transfer (master PC's earned CDP can be spent on the Apprentice instead) is a separate followup feature, not part of creation.
