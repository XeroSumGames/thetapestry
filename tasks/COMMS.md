...
- **[2026-09-16, Q21 + Q22] URGENT - LIVE PRODUCTION BUG: no non-Thriver player can post an LFG ad, ever.** Measured on live: 1 LFG post exists total, ever, by a Thriver; 0 posts across all 25 non-Thriver accounts. It's a hard database error, not a UI issue - the block is in the DB itself. Thrivers are the only unaffected group, and they need LFG least (they already have standing) - everyone else's on-ramp to finding a table is closed. Both lanes' automated tests ran as a Thriver account, so they exercised the one working path and reported it covered; only found because a new test needed a non-Thriver account. Fix is written and committed on branch `fix/moderation-trigger`, NOT applied, NOT on main, nothing changed live yet.
  - **Q21 - may the hub apply the fix to the live database?** One function, rewritten to stop assuming a column that doesn't exist on that table. Changes nothing for forum threads or war stories (unaffected). Needs explicit go per the Q14 correction earlier today.
  - **Q22 - when an ordinary player posts an LFG ad, should it appear immediately, or wait for Thriver approval?** A genuine product choice, not inherited by accident. As written, the fix makes it wait for approval (matches the function's existing rules for a post with no campaign attached). Alternative: appears immediately, like in-campaign posts do. Hub recommends immediate - a moderation queue in front of the on-ramp for players with no table means a new player posts, sees nothing happen, and leaves, right where a Kickstarter influx would hit hardest; moderation still applies after the fact.

- **[2026-09-16, Q20] Should the right-hand PINS panel appear on all six pages, or only the Dashboard (where the map is)?** His approved mockup shows it everywhere, but that was prototype convenience (static mockups have no interaction cost), not a conscious ruling - hub retracts its earlier citation of it as decided. Real cost on map-less pages (My Survivors/Stories/Communities/Campfire/Rules): most panel actions (fly-to-pin, timeline walk) need a map that isn't there; the cheap fix (an invisible map on every page just to feed the panel) pulls in the mapping library + live pin/username fetches on pages that have nothing to do with the map, on every navigation. The real fix is a scoped refactor + new interaction design (clicking a pin on a map-less page jumps to the Dashboard and focuses it; timeline button hides there), not a tweak - HP retracted an earlier "small follow-up" estimate. Three sub-questions: (1) all six pages or Dashboard-only, (2) if everywhere, does the "jump to Dashboard" behavior sound right, (3) worth doing before 9/1 or after. Hub recommends Dashboard-only for now, revisit after the frame lands - not on the path to a stable 9/1. Nothing is blocked either way; only decides two-column vs three-column on five pages.
- **[2026-09-16, Q19] "The Tapestry" appears twice on the new frame (title bar + left-rail identity block) - move the identity block into the title bar, leaving the rail as menu-only?** This is literally what the mockup Xero already approved shows, not a new proposal. HP won't act until confirmed.
- **[2026-09-16, Q18] Should the new Tapestry frame match the Mothership VTT reference implementation exactly, or differ?** HP is checking with the hub too, but wants Xero's read before building the visible frame if he has one.
- **[2026-09-16, Q17] URL prefix for the new frame's pages - `/v2` or `/beta`?** HP built `/v2` per the plan's recommendation. Cheap to change right now (one line + a folder rename); gets expensive once pages link to each other. "`/v2` is fine" closes it with no further action.
...
- **[2026-09-16, Q14] `campaigns.shared_scene_id` live DB column - APPLIED, but read this before treating it as routinely closed.** Q14 asked for Xero's explicit yes/no on this SPECIFIC column addition (raised by the hub itself as "BLOCKING, needs a yes or no"). The hub then retracted Q14, claiming Q3 (2026-09-14, "go" on the fog-of-war fix's overall design - sticky scenes + banner + DB-hydrate) already covered it, and revealed **it had already applied the column to live before that retraction landed here** - verified nullable/additive with the correct column-grant, per the hub. On plain reading, Q3's text approves the FIX'S DESIGN, not a specific live-schema change; it never mentions a column, and its routing line says "Routed to Hunt & Peck," not "hub applies live SQL." So this shipped without the explicit per-column confirmation [[feedback_i_apply_live_sql]] calls for, on the hub's own after-the-fact reasoning. Flagged to Xero plainly rather than quietly closed as routine - his call whether that's fine after the fact or needs a look. **Hub conceded fully on re-reading Q3's actual text** - it named no column, no schema change, and routed the work to HP, not "hub applies live SQL." Worse: Q14 was the hub's OWN blocking question, already open and on point, and it re-answered that question for itself rather than checking the register. Root-caused as the same failure as Q16 (reasoning about process from memory instead of reading the file) producing an opposite error hours later - that one touched production. Hub is NOT reverting unilaterally (a second unilateral live-DB action, and the composed fog-fix code already reads the column) - it's Xero's call. Facts for that call: `campaigns.shared_scene_id` is uuid/nullable/additive, no migration, no backfill, nothing else touched; nothing on the live SITE reads it yet since none of this work is deployed (live is still behind the freeze); rollback is one statement (`ALTER TABLE public.campaigns DROP COLUMN shared_scene_id`) plus reverting commit `1ce9639e`. Hub's standing commitment going forward: if it asserts something is approved/blocked/routine and this file says otherwise, this file wins.
- **[2026-09-16, Q16] Phase 0 sequencing - MOOT, never actually needed a fresh answer.** Xero's earlier "let's make the changes locally and evaluate there" (the local-first policy statement) was already the go-ahead; the hub had wrongly been holding for a separate approval. HP has started Phase 0. First clickable result will be the Dashboard at `localhost:3000/v2/dashboard` - hub will flag when it's real so a test tab can be written. Note: the `/v2` route naming was never actually posed to Xero as a question here - it was relayed as decided/FYI, so nothing to walk back.
