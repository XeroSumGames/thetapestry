...
- **[2026-08-04] Content needed: onboarding video for the new stepped-modal first-timer tour (Puffer Fish).** Xero asked for a redesigned new-player onboarding flow (7-step popup sequence, routed to HP to build - see `todo.md`/session log for full spec). Step 7 is "watch this video" - grepped the app for an existing tutorial/trailer video wired anywhere in onboarding and found none (only an unrelated video reference on the press-kit page). **Ask Xero: is there an existing video to use (and where), or does one need to be made/sourced before this step can be built?** Not blocking the other 6 steps - HP's building those now with a placeholder slot for this one.
