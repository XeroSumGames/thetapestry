...
- [ ] **Stabilize / Distract / Gut Instinct Phase 4 - retire legacy executeRoll branches.** Once the 2026-05-25 playtest verifies all three dedicated modal paths are clean, delete the unreachable `if (pendingRoll.label.includes('Stabilize '))` + `if (pendingRoll.label.endsWith(' - Distract'))` + `if (pendingRoll.label.includes('Gut Instinct'))` blocks in `app/stories/[id]/table/page.tsx` (~85 lines combined). Mark in commit as "Phase 4 cleanup".
