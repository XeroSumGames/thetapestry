...
- [ ] **Phase 2 (3 left of 7):** ~~char-create methods~~ (#8) + ~~session lifecycle~~ (#9) + ~~rumors publish->clone~~ (#14) + ~~NPC roster CRUD~~ (#11 core) DONE; remaining: communities lifecycle + weekly check (#12), inventory/trade (#13), combat flow+propagation (#10 - heaviest + partly dice-gated/manual, do it LAST + coordinate its table-page testids with Hunt & Peck). All shipped so far needed ZERO app testids (REST/DOM via existing title= attrs).
