...
- [x] **[FIXED 2026-06-16 `550e252`][HIGH] Charge "Cancel Move" didn't cancel the Charge** - Xero marked this twice: "charge when canceled is broken" (00:51:28 UTC) and "charge failed again - hit charge, hit cancel, still got the CHARGE modal and two moves" (01:01:45 UTC). Root cause: `page.tsx:6211` "Cancel Move" button onClick inlined `setMoveMode(null)` but NOT `pendingChargeRef.current = null`. Charge stayed live invisibly; the next Move click triggered the Charge attack roll modal unexpectedly, then after cancelling the roll, Move appeared again for a second free move. Fix: route "Cancel Move" button to `handleMapMoveCancel()` (which already clears all three: `pendingChargeRef`, `sprintPendingRef`, `moveMode`). One-line change, 875 tests green. Testplan: [tasks/charge-cancel-testplan-2026-06-16.md](charge-cancel-testplan-2026-06-16.md).
