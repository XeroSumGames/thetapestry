...
### 🎨 Modal VISUAL unification (DEDICATED LANE; spec [tasks/modal-visual-unification-spec-2026-05-21.md](modal-visual-unification-spec-2026-05-21.md))
Make every dice-roll/check modal share ONE visual shell identical to the canonical ATTACK ROLL modal, preserving each modal's functional body. Distinct from the *functional* modal-unification (line ~286, COMPLETE) which migrated rolls onto `<RollModal>`; this is the visual reskin of that shared shell. Testplan: [tasks/modal-phaseA-testplan.md](modal-phaseA-testplan.md).
- [x] **Phase A** SHIPPED 2026-05-24 (`8cc173f`). Reskinned `components/RollModal.tsx` to the ATTACK shape: 400px draggable panel (new `lib/use-drag-position.ts` hook), contextual backdrop (`dimBackdrop` prop - no-dim over the tactical map, dimmed over a sheet), feed palette (`outcomeColor`, Success=blue) replacing the divergent green-Success `MODAL_OUTCOME_COLOR`, per-roll `accent` (eyebrow + Roll button), ATTACK-faithful dice tiles (52px / 2px / 28px / #242424) + buttons. All 8 callers wired with `eyebrow`/`accent`/`dimBackdrop`. Accents: Recruit `#b07cc6`, Stabilize `#7fc458`, Distract `#4aa3b5`, Gut Instinct `#5aa0c0`, Vehicle `#d4883a` (no-dim); Stress `#EF9F27`, Breaking Point + Lasting Wound `#c0392b` (dim). Gates: tsc + font/role/em-dash/arch + 568 tests. **Owed: Xero's per-modal visual eyeball on the deployed dev env (testplan above) - NOT browser-verified by Claude.** Watch item: Vehicle check renders no-dim over a non-map page; may want `dimBackdrop` if it reads unanchored.
- [ ] **Phase B** - reskin `app/stories/[id]/table/components/FirstImpressionModal.tsx` (biggest outlier: 480px, custom red border, pick/rolling/result phases) onto the canonical shell.
- [ ] **Phase C** - reconcile the inline ATTACK modal (`page.tsx:7819`) to consume `lib/use-drag-position.ts` + align residual spacing so ATTACK and the component are pixel-identical (mostly de-dup; ATTACK already uses `outcomeColor`).
- [ ] **Phase D (DEFERRED, post-playtest, Xero-gated)** - collapse the inline ATTACK modal into the shared `RollModal` component. Load-bearing on the combat surface; do NOT before a playtest passes.
...
- [ ] **[LOW] Live em-dashes in DB function bodies (surfaced by the 2026-05-23 schema capture).** `sql/_baseline/schema.sql` revealed 8 em-dashes inside LIVE function/trigger bodies - incl. a USER-FACING notification string in the community-growth notifier (the "has grown to N members [U+2014] it's official" line uses a literal em-dash) + several RAISE NOTICE / comment lines. The user-facing one violates the no-em-dash rule IN PRODUCTION (reaches users). Fix = editing live function bodies = schema change (bright-line: confirm before applying). Hunt-and-peck or a dedicated pass. Lines: schema.sql:3293,3341,3808,3907,4003,4514,4956,5032 (4514 + 4956 are the user-facing notify strings).
...
- [x] ~~**[MEDIUM] (operator, Xero)** Apply `sql/audit-log-table-2026-05-20.sql` to live~~ DONE 2026-05-24 (Xero ran it). Audit-log table now live (recovery/forensics substitute for deferred Pro/PITR).
