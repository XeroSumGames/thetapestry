...
- [ ] **[PARKED - needs Xero canon go] Intimidation not in canonical PC skill list.** `lib/xse-schema.ts` SKILLS has no Intimidation, but the recruitment/morale smod (`max(Intimidation,Inspiration,Psychology*,Tactics*)`) + many NPCs reference it -> `skLevel('Intimidation')` is always 0 for PCs. Decoupled from the pregen feature. If builder-made pregens should train it, add to SKILLS (attribute: old sheet says PHY, recommend INF). David's existing entry folds his Intimidation 2 into Manipulation 3.
