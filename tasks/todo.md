...
- [~] **Grapple/Subdue/Break-Free rework** - design locked with Xero; canon updated (`app/rules/combat/combat-rounds/page.tsx:26`, `ebb19ce`); plan + verified-state in [tasks/grapple-subdue-rework-spec.md](grapple-subdue-rework-spec.md).
  ...
  - **NEXT - Phase 2b Break Free** = opposed, grappler reactive (no action), +1 RP on defender fail, ends on success (replace the "reopen grapple modal" wiring at the Break Free button). Then **Phase 3:** `initiative_order.pending_action_loss` column (Xero-approved live DB change) for "lose next action whenever it falls" (the carryover edge `consumeAction` swallows today). Then **Phase 4** modal polish + the deferred executeRoll consolidation onto `applyDamageToPc/Npc`.
