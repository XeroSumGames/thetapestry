...
- [ ] **[E2E P1] `grapple-subdue-breakfree.spec`** - Extends grapple-family: REST poll defender's `actions_remaining` decrements on successful grapple; Subdue + Release buttons both visible in action bar while `isGrappling`; Break Free reduces breaker's `actions_remaining`. All REST/DOM, no canvas. (~45 min)
- [ ] **[E2E P1] `character-card-trackers.spec`** - WP/RP/Stress click persists via REST poll; 0 WP -> stress auto-bump: call `gm_apply_damage` to wp=0, assert `stress = min(5, before+1)`. (~30 min)
- [ ] **[E2E P1] `env-damage.spec`** - GM clicks Falling (3 WP / 3 RP per 10 ft) + Drowning (3 WP / 3 RP per round) ENV buttons; assert `character_states` decremented by canon amounts via REST. (~20 min)
