...
- [~] **[PUFFER 2026-05-31] AUDIT M3 - `console.*` sweep top 4 files - Phase 1 SHIPPED (CampaignCommunity).** Pattern locked: Supabase-error-shape `console.error` calls migrate to `reportSupabaseError(err, context)` from `lib/supabase-errors.ts` (preserves dev console.error + adds Sentry capture with `supabase_error_code` tag). Domain-assertion console.errors (non-Supabase-shaped) stay as-is. CampaignCommunity 10 -> 2 (the 2 remaining at `:810` and `:1208` are domain assertions, legitimate keeps). Phases 2-4 owed: useRollResolution (19 sites), campaign-clock (17 sites), table-page (51 sites). Pattern proven; HP can apply to the remaining 3 files using the same triage (Supabase-shaped → reportSupabaseError; non-Supabase domain log → keep). Road-to-9-1 AUDIT M3: 1 of 4 files cleared.
...
- [ ] **[MEDIUM][HP] AUDIT M3 - console.* sweep** top 4 (table-page 51, useRollResolution 19, campaign-clock 17, CampaignCommunity 10). Replace prod-relevant logs with Sentry breadcrumbs; remove dev-only logs.
