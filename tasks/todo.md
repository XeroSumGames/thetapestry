...
7 GM-typed notes pulled from the GM dump. Routed by lane. (Players' dumps had 0 marks; grumpsterfier = 0 events over 45min = Enya's player effectively locked out by the tactical-map bugs above. GM also hit the locked tactical map throughout - already routed.)
...
- [ ] **[PUFFER to INVESTIGATE -> realtime/RLS vs client] Campaign pin didn't reveal to players on approach (GM note #4).** Verbatim: "pin didnt' show to players as they neared it". A proximity-reveal `map_pins` pin stayed hidden from players who neared it. Puffer: check the pin reveal write + realtime propagation + RLS (does a player SELECT a not-yet-revealed pin? does the proximity-reveal write happen + broadcast?). If the gap is client proximity-detection logic, route to HP. (Puffer-adjacent because map_pins realtime/RLS/triggers are this lane.)
