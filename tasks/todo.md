...
### 🚨 E2E test-infra regression (HIGH, discovered 2026-06-01)
- [ ] **[ROUTED -> HUNT & PECK 2026-06-01][HIGH] combat-flow Phase A "Start Combat" UI path failing on prod** - the `Start Combat` action-bar button never appears after the `Start Session` click that worked at the 2026-05-30 138-passed re-cert. Blocks `combat-flow.spec` Phase A re-runs AND any new spec that exercises Start Combat (e.g. `e2e/grapple-family.spec.ts` shipped today + parked under `test.fixme`). Three plausible root causes (HP investigation needed to pick one): (1) Start Session click silently failing post the rest-finish A+B+C work (`5ba32d1`/`e468345` touches session machinery); (2) Start Combat button text changed; (3) new precondition gate the test's existing setup doesn't satisfy. Finding + screenshot pointer: [tasks/finding-combat-flow-phase-a-start-combat-broken-2026-06-01.md](finding-combat-flow-phase-a-start-combat-broken-2026-06-01.md). On fix: ping E2E -> un-fixme grapple spec + re-cert Phase A in one batch; Subdue + Break Free contracts layer on top.
...
- [x] **[ROUTED -> HUNT & PECK 2026-05-31] hidden NPCs in initiative (GM-only, no-spoiler combat prep)** - SHIPPED `ac27b8a` 2026-05-31. `initiative_order.hidden_from_players` column live (SQL `ed1f76e`); DB types updated; NpcRoster combat picker gets "Hidden from players" checkbox; InitiativeBar: non-GM sees filtered list + "Waiting..." placeholder on hidden-active turn; GM sees full list with amber HIDDEN chip + 👁 Reveal button; Reveal writes `hidden_from_players=false`, makes scene_token visible, posts reveal roll_log row. Seam functions `revealInitiativeEntry` + `makeTokenVisibleByNpc` in `lib/data/tactical.ts`. Finding: [tasks/finding-hidden-npc-in-initiative-2026-05-31.md](finding-hidden-npc-in-initiative-2026-05-31.md).
  - **[E2E QUEUED]** multi-client visibility spec: GM adds NPC with `hidden_from_players=true` -> player's InitiativeBar does NOT render the entry AND player's TacticalMap does NOT render the matching scene_token -> GM clicks 👁 Reveal -> both render on player side within realtime SLA AND a reveal `roll_log` row lands. Covers both PC and NPC observer angles. Will build once HP's UI ships.
