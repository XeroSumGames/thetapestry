...
- [x] **[ROUTED -> PLAYWRIGHT/E2E] build combat-flow.spec.ts Phases A + C - DONE 2026-05-30.** Both phases green standalone (27s combined) AND in the full re-cert. Phase B (initiative-bar DOM ordering / action decrement) still waits on the 4 HP testids (separate ask once HP wraps move-follow); end-of-combat infection-modal trigger needs a small RPC extension (Puffer can add `p_infection_risk boolean DEFAULT false` to `gm_apply_damage` OR a sibling `gm_set_infection_flag` RPC - ping Puffer with the preferred shape when needed). Plan: [tasks/e2e-combat-flow-plan-2026-05-30.md](e2e-combat-flow-plan-2026-05-30.md).
