...
- [x] ~~**NPC ACU (Acuity) attribute schema gap**~~ — COLUMN ADDED 2026-05-15 (`sql/npc-lasting-wounds-and-acuity.sql`). `campaign_npcs.acuity smallint NOT NULL DEFAULT 0`. Vehicle popout's NPC mapper now reads it into the navigator's `attributes.ACU` so the Navigate skill picker's ACU-based skills (Navigation, Farming, Gambling, Lock-Picking) read the right AMod. Existing NPCs default to ACU=0 (matching their previous effective value); per-NPC backfill via the NpcCard editor when Xero has time. UI surface for editing ACU on NpcCard not in this commit - still defaults to 0 for any NPC created via the existing editor.
- [ ] **Surface ACU editing on NpcCard / setting-npcs editor** — column added 2026-05-15 but no UI to set it. NPCs default to 0 ACU until Xero edits the column directly or wires the editor.
- [ ] **NPC `lasting_wounds` UI / NpcCard surface** — column added 2026-05-15 (`campaign_npcs.lasting_wounds jsonb`). Drainer + executeRoll now write to it. NpcCard doesn't display the array yet (PCs already show their `lastingWounds[]` on the CharacterCard).
