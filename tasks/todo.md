...
- [~] **Grapple/Subdue/Break-Free rework** - design locked with Xero; canon updated (`app/rules/combat/combat-rounds/page.tsx:26`, `ebb19ce`); plan + verified-state in [tasks/grapple-subdue-rework-spec.md](grapple-subdue-rework-spec.md).
  ...
  - **NEXT (hot-file wiring, fresh focused pass):** add `grappleMode` to the grapple modal; **Subdue** = opposed PHY+Unarmed vs PHY+Unarmed/Athletics, on win deal 1 WP + (1d3+PHY+Unarmed) RP via `applyDamageTransition` (PC + NPC emission); fists-only (drop melee substitution at `:5759`). **Break Free** = opposed, grappler reactive (no action), +1 RP on defender fail, ends on success (replace the "reopen grapple modal" wiring at `:5737`). **Phase 3:** `initiative_order.pending_action_loss` column (Xero-approved live DB change) for "lose next action whenever it falls." Needs a live-verify pass + testplan (hot-file UI, no unit-test net) - same standard as the grapple-modal migration this session.
