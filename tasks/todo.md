...
- [ ] **[MEDIUM] Conditions subsystem** - unify infection / lasting-wound / stress / MW into one model + one render (the chips) + one reset (Restore). Kills the recurring bug class this session kept hitting.
- [ ] **[MEDIUM] Infra-as-code** - publication membership + RLS + triggers + the 15 orphan-table schemas into versioned CI-applied migrations (closes the silent-config-bug class; the publication gap that cost an hour today).
- [ ] **[LOW] Typed payloads + de-regex the feed** - `DamagePayload`, structured roll_log columns (pays down `as any` x574 + label-parsing).
...
- [ ] **[MEDIUM] Publication gap - held tables triage.** Same root cause as the 2026-05-24 fix, NOT yet added (need scope confirm): `characters` (high update frequency + RLS implications), `war_story_replies`, `forum_thread_reactions` (campfire/forum surfaces). Decide per-table whether realtime is wanted before `ALTER PUBLICATION ADD`.
- [ ] **[HIGH] Batched Phase 7 2-client acceptance** across vehicle / NpcRoster / community+stockpile / MapView realtime + the end-of-combat infection modal (only TacticalMap + presence smoked so far). This is the demote-gate for the Realtime YELLOW + the Grand Re-Arch finish line. Needs Xero (two windows). **ONE runnable sheet: [tasks/phase7-acceptance-2client-testplan.md](phase7-acceptance-2client-testplan.md)** (Sections A-F; supersedes the table-only `decomposition-2client-smoke-testplan.md`, whose 3 KNOWN-FAILUREs are now fixed + re-verified in Section A).
...
- [x] **[MEDIUM] UX: status condition chips in the player bar.** SHIPPED 2026-05-24 (Xero "yes to all"). Player-bar avatars now show condition chips under the name, visible to EVERYONE (read from liveState, kept fresh by the character_states realtime sub): 🦠 Infected/Sick (+days), 💀 MW (+countdown), 😵 Incap, 😰 Stressed (stress>=5). `app/stories/[id]/table/page.tsx` ~7735. Fixes the invisibility that made the infection bug look like a re-arch failure for an hour.
  - [ ] **FAST-FOLLOW (HUNT-AND-PECK): 🩹 Lasting Wound chip + tooltip refinements** (the 5th chip). Handed to hunt-and-peck 2026-05-24 (tactical UI). Lasting wounds are FEED-DERIVED (roll_log "🩸 Lasting Wound:" rows, parsed in CharacterCard.tsx:343), not a `character_states` column - so the player bar needs a `Set<characterName>`/`Map<name, woundText>` derived from the feed (or a future `lasting_wounds` column). Add the chip in `app/stories/[id]/table/components/PlayerStatusChips.tsx` with a `title` showing the actual wound. Also free to refine the existing 4 chips' tooltips/styling. See the chat handoff block 2026-05-24.
- [ ] **[MEDIUM] (operator, Xero)** Apply `sql/audit-log-table-2026-05-20.sql` to live (recovery/forensics substitute for deferred Pro/PITR). Bright-line: live-DB migration, confirm intent.
...
### UI: fold Advantages tab into Notes + redesign Notes (HUNT-AND-PECK; GATED on table freeze + re-arch)
- [ ] **Move the Advantages tab INTO the Notes tab, and redesign the Notes tab to be more functional.** (Xero 2026-05-21.) The right-panel tab strip lives in the frozen table page: tab labels at `app/stories/[id]/table/page.tsx:8977` (Pins / NPCs / Assets / ⭐Advantages / Notes); Advantages panel content ~9580-9601; Notes renders `components/GmNotes.tsx` (GM) / `components/PlayerNotes.tsx` (player) at ~9509-9510. Two parts: (1) fold the ⭐Advantages panel into the Notes tab and drop the standalone Advantages tab; (2) redesign Notes to be "more functional" - needs a design pass (clarify scope with Xero when picked up: inline editing? search? per-character notes? pinning?). GATED: this is table-page work, blocked by the TABLE FREEZE above, and the re-arch is restructuring this exact file - wait for the freeze to lift, then reconcile against wherever the re-arch relocates these tabs (likely extracted components). UX/design item, not a quick edit.

### Recruitment: recruit into a GROUP, not a forced Community (HUNT-AND-PECK; CANON CONFORMANCE; GATED on table freeze + re-arch)
- [ ] **PCs must be able to recruit NPCs WITHOUT founding/naming a community.** (Xero 2026-05-21.) This is a code-vs-canon bug, not a new rule. Canon (`tasks/tapestry-rules-canon.md:746`): "PCs working together are a **Group**. Players recruit NPCs to their Group via a Recruitment Check. If a Group grows to a combined total of **13 or more** PCs and NPCs, it becomes a **Community**. Communities require regular Morale Checks; Groups do not." The current Recruit pick-step modal violates this: with zero communities it forces "this recruit will found a new one" + a NAME field + a Make-Public toggle (see Xero's screenshot). Correct behavior: recruit into the PC's **Group** with no name / no public toggle / no Morale-Check apparatus; the Group only becomes a named **Community** at the 13th combined member (PCs + NPCs). Two sides: (1) UI - recruit flow drops the found-a-community step until the 13-member threshold (recruit pick-step modal is in the FROZEN table page, pick step ~`app/stories/[id]/table/page.tsx:12439`, result shell ~12710); (2) DATA MODEL - recruited NPCs currently land in `community_members` tied to a `communities` row, so a pre-13 "Group" needs a home that does NOT require a `communities` row (schema question - likely a Group entity or a draft/proto-community; SCHEMA-TOUCHING = always-confirm-first bright line, do not implement the migration without Xero's go). Canon is already correct, so NO rules-page change needed - this is making code conform to canon. GATED: table-page UI work blocked by the TABLE FREEZE + re-arch; coordinate the data-model change with the Communities flagship system (Phases A-E shipped). Confirm the 13-member transition UX with Xero when picked up (what happens AT member 13 - prompt to name the community?).

### Presence count: show everyone, including users in a /table (HUNT-AND-PECK; NOT frozen-blocked - global components)
- [ ] **"Survivors present: N" undercounts - anyone inside a /table is missing from the count.** (Xero 2026-05-21, screenshot.) Root cause: the `global_presence` Supabase channel + `.track()` live INSIDE `components/Sidebar.tsx` (channel created ~`:85`, `track({ user_id })` ~`:100`), but `components/LayoutShell.tsx` hides the Sidebar on full-width routes - `FULL_WIDTH_PATTERN` (`:20`) matches `^/stories/[^/]+/table$`, so `hideSidebar` is true (`:197`) and the Sidebar (hence the presence tracker) never mounts while a user is at the table. Result: table users don't `track()` and vanish from the count. Fix: relocate the presence `track()` to an always-mounted layer (LayoutShell itself, or a dedicated presence provider) so it runs on EVERY authenticated route regardless of sidebar visibility; the count + Thriver hover-list display can stay in Sidebar. NOTE: this fix is OUTSIDE the frozen table zone (global components only - no `app/stories/[id]/table/**` edit needed), so it is shippable during the freeze WITHOUT colliding with the re-arch if Xero greenlights. Verify with a 2-client test (one opens a /table, the other watches the count include them).

### /record dumps should save to C:\TheTapestry\playtestlogs, not Downloads (HUNT-AND-PECK; NOT frozen; has a browser constraint)
- [ ] **Redirect `/record` playtest-log downloads to `C:\TheTapestry\playtestlogs`.** (Xero 2026-05-21.) Source: `lib/playtest-recorder.ts:224` `downloadDump()` - builds a Blob + sets `a.download = name` and clicks it, so the file lands in the browser's default Downloads folder. CONSTRAINT (honest flag): a web page CANNOT write to an arbitrary absolute disk path - `a.download` only sets the filename, not the directory (browser security). Two feasible routes: (a) **File System Access API** - `showDirectoryPicker()` lets Xero pick `C:\TheTapestry\playtestlogs` once; persist the returned directory handle in IndexedDB and write every subsequent dump there with no re-prompt. Chromium-only (Chrome/Edge), needs a user gesture + one-time permission grant. This is the real in-app fix. (b) **Zero-code workaround available NOW:** set the browser's default download location to `C:\TheTapestry\playtestlogs` (or enable "Ask where to save each file") - applies to all downloads, not just /record. NOT FROZEN: `lib/playtest-recorder.ts` is telemetry lib, outside the table freeze (the `/record` toggle `useRecorderToggle.ts` is in frozen table hooks, but the dump fn is not). Could ship route (a) during the freeze if Xero wants - confirm Chromium-only is acceptable.

### Infection: only from cut/shot wounds, not punches/blunt (HUNT-AND-PECK; GATED on table freeze - damage path; canon reconcile)
- [ ] **Wound Infection must only trigger for skin-breaking wounds (cut / shot), not unarmed/blunt.** (Xero 2026-05-21; screenshot: an "Unarmed Combat" hit fired "🩸 Wound Infection Warning".) Current behavior: the warning is queued for ANY WP damage regardless of weapon - `app/stories/[id]/table/page.tsx:5449` (PC target) + `:5565` (NPC target), both `if (finalWP > 0) pendingWoundInfectionRef.current.add(...)`, no weapon-type gate. Fix: gate the queue on whether the weapon breaks skin. At minimum exclude Unarmed (`weapon.weaponName === 'Unarmed'`); decide whether blunt melee (Club / Bat / etc.) also counts as non-infecting (Xero said "punched" - confirm blunt-weapon scope). Cleanest: add an infection-eligible / "breaks skin" property to the static weapon table (`lib/weapons.ts` - data, not DB schema; bladed/piercing/firearm = yes, blunt/unarmed = no) and gate both queue sites on it. CANON: reconcile with Infection canon (CRB p.114-115; `project_infection_canon`) - confirm whether canon already restricts Wound Infection to skin-breaking wounds or this is Xero's clarifying ruling (if a ruling, add it to canon). GATED: the queue sites are in the frozen `executeRoll` damage path (re-arch is extracting useRollResolution - reconcile after freeze); the weapon-data classification in `lib/weapons.ts` is not frozen, but the gate logic is.

### Log wording: GM loot-distribution note is ugly (HUNT-AND-PECK; GATED on table freeze)
- [ ] **Clean up the GM "give items to PCs" loot log line.** (Xero 2026-05-21; screenshot showed `🎒 Loot:  → Cree Blaine`.) Source: `app/stories/[id]/table/components/LootModal.tsx:111` - `` label: `🎒 Loot: ${itemList} → ${names}` ``. Two problems: (1) the format reads clunky ("Loot: X → Y") - reword to something cleaner (e.g. "🎒 Cree Blaine received <items>" - confirm exact wording with Xero); (2) the screenshot shows an EMPTY `itemList` even though the give is guarded against zero items (`:88` early-return, button disabled `:117`), so an item with a blank/empty `name` is slipping through `lootItems.map(i => i.name...)` (`:108`). Fix the format AND guard blank/empty item names so the arrow never renders with nothing before it. GATED: `app/stories/[id]/table/components/LootModal.tsx` is in the frozen table zone (the re-arch extracted LootModal as a component - reconcile after freeze). Sibling loot lines for wording consistency: "🎒 <name> looted <items> from <object>" (auto-loot, page.tsx + CampaignObjects.tsx).

### Log wording: wound-infection "shrugged off" line (HUNT-AND-PECK; GATED on table freeze - roll code)
- [ ] **Reword the no-infection (success) wound-infection log line.** (Xero 2026-05-21.) `lib/roll-helpers.ts:191` (the `shrug` branch, `kind === 'Wound'`) currently reads `` `${r.character_name} shrugged off the wound infection${outcomeTag}` ``; change to `` `${r.character_name}'s wound does not become infected${outcomeTag}` ``. Example output: "Tucker Ortega's wound does not become infected". OPEN: the parallel Sickness line at `:192` ("shrugged off the sickness") - Xero to confirm whether it changes to match (e.g. "does not become sick"). GATED: `lib/roll-helpers.ts` is the table's roll-narrative code (frozen). PREVIEW-SYNC: this change MUST update `tasks/roll-feed-log-preview.html` in the same commit (pre-commit guardrail enforces).
...
- [ ] **Y11-e `roll_log` session-archive** - SPEC READY: [tasks/spec-roll-log-session-archive.md](spec-roll-log-session-archive.md). 5 phases RLA1..RLA5 (schema -> stamp session_id via saveRollToLog -> filter feed + remove DELETE -> per-session browse -> chat_messages decision). `session_id uuid REFERENCES sessions ON DELETE SET NULL`. No backfill (pre-migration rolls stay NULL). 4 risks; RLA-R2 (ship-order) is the key one - stamp before filter.
