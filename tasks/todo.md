...
- [x] **[VERIFY-FIRST SWEEP 2026-05-31] HP pickup #5 Travel Times subsystem - already SHIPPED `e7b1e56`.** `lib/travel.ts` implements 8h soft-cap + 1 RP per push hour, returns `{rpCost, pushHours}` for feed labels. Wired into `components/CharacterCard.tsx:642` Travel action. 10 unit tests green. Verify pass owed: confirm the campaign-map route-planner (if it exists) uses `travelPushCost()` for multi-hour route planning; small wiring task if it doesn't. Audit doc: [tasks/canon-extract-mechanics-status-2026-05-31.md](canon-extract-mechanics-status-2026-05-31.md).
- [~] **[VERIFY-FIRST SWEEP 2026-05-31] HP pickup #3 Item Condition + Upkeep - SHIPPED `724a1e2`; spot-check owed.** Upkeep Check button at `components/CharacterCard.tsx:1005-1025`; outcome handler at `lib/roll-helpers.ts:577-585` narrates degrade + irreparable damage; 33 unit tests cover Pristine -> Used -> Worn -> Damaged -> Broken transitions + Wild Success cap-at-Used. Spot-check owed: confirm Broken weapons actually REFUSE to fire in the live combat path (not just narrated) - check the weapon-attack click handler in `app/stories/[id]/table/page.tsx` for a condition gate. 15-min verify, not a build. Audit doc: [tasks/canon-extract-mechanics-status-2026-05-31.md](canon-extract-mechanics-status-2026-05-31.md).
