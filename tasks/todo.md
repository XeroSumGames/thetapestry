...
- [ ] **Phase 1 remaining (3):** presence ("Survivors present: N"), world-pin-to-queue (moderation), account-settings. (LFG "I'm Interested" + war-story composer also still uncovered - the moderation trigger is shared so the role-gate generalises, but those composers' UI flows are untested.)
...
- [x] **Re-cert fix - `section-c-npc-reveal` hardened + `playwright.config` retries:2** (2026-05-24). A full-suite re-cert caught section-c red (was green standalone): it picked `campaign_npcs limit 1` with NO ORDER BY (non-deterministic) and asserted whole-page `getByText(name).toHaveCount(0)` while hidden - broke when it grabbed "Hugo Vale" (name renders 24x in relationship lists, not gated by the roster's hidden flag). NOT a leak (roster DOES filter hidden_from_players; confirmed - revealing strictly increases the count). Fixed: deterministic `&order=id.asc` + DELTA assertion (leak-proof). Also set `retries: 2` in playwright.config (was 0 locally): full-run flakes (transient prod 500s incl. the Sentry /monitoring tunnel; Section-D D-3 resubscribe timing under load) now pass on retry while a REAL regression fails every attempt. Final cert: **111 passed / 0 failed**.
- [ ] **WATCH - flaky-under-load routes:** `/creating-a-character` + `/rules/core-mechanics/negotiations` (sweep, transient 500s) and `section-d` D-3 (realtime resubscribe timing) each flaked once in a full run, all green on isolated retry. Retries absorb them. If D-3 keeps flaking, confirm it's only test-load sensitivity and not a real-world resubscribe raciness.
- [ ] **Phase 2 (7):** char-create methods, session lifecycle, combat flow+propagation, NPC roster CRUD + recruit gates, communities lifecycle + weekly check, inventory/trade, rumors publish->clone (+ ~5 small `data-testid`s).
