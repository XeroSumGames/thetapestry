...
- [ ] **[ROUTED -> HUNT & PECK] NPC sheet: support multiple weapons** (Xero 2026-05-27). Current model: one primary weapon + one optional secondary (Foe/Antagonist only). Needed: arbitrary N weapons per NPC (e.g. a machete + a pistol for any tier). Bundled with the ADD NPC modal redesign - both touch the weapon data model (`skills.weapon` / `skills.weapon2` today; needs to become `skills.weapons[]` array). Data model change = coordinate with Puffer Fish (schema + loot RPC `loot_npc_equipment_item` reads by slot name, will need updating when the array lands).
