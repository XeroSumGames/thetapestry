...
- [ ] **PERF (Phase 3 useInitiative target): `nextTurn` is slow (656ms-3,638ms per turn advance).** Observed in the 2026-05-22 Part 2 smoke (`[nextTurn] done` traces). It's sequential DB round-trips: `deactivate_ms` + `activate_ms` + `reload_ms` run in series (worst case activate 1849 + reload 1406 + deactivate 384 = 3.6s). The `useInitiative` rebuild (grand plan Phase 3) should batch these into one wave + apply optimistic local state so the turn flips instantly and the writes settle in the background (same pattern already used in the Restore flow). NOT a correctness bug; nothing dropped. Disposition: optimize during the rebuild, not now.

- [ ] **BUG (realtime view-state, Phase 3 useTableRealtime/useTacticalSync): while the tactical map is SHARED, the GM cannot switch their own view back to the Campaign map.** Found by Xero, 2026-05-22 (decomposition smoke Part 3, tactical-share step) - "GM couldn't get back to campaign map BUT was able to share map"; click log shows ~8 Tactical<->Campaign toggles in 30s before unsharing freed it. Root cause confirmed in code: the GM view toggle (`page.tsx:7276`) flips `showTacticalMap` but leaves `tacticalShared` true; meanwhile `setShowTacticalMap(true)` is force-fired by the `scene_activated` handler (`:1351`, gated on `tacticalSharedRef.current`) and the `tactical_shared` handler (`:1340`), so any echo/scene event snaps the GM back to tactical. Net: sharing pins the GM to tactical until they Unshare. **DESIGN DECISION NEEDED (Xero):** should the GM be able to preview the Campaign map while players see the shared tactical map? Almost certainly yes (GM plans on campaign while combat tactical is shared). Fix in the Phase 3 rebuild: decouple the GM's LOCAL view intent from the shared-state force (sharing should drive what PLAYERS see, not override the GM's own pane). DISPOSITION: fix during Phase 3 (useTacticalSync + useTableRealtime), NOT now - it lives in the realtime handlers being rebuilt. Add to the fix-do-NOT-preserve list. Cross-ref: [tasks/grand-rearchitecture-2026-05-22.md](grand-rearchitecture-2026-05-22.md) Phase 3.

- [ ] **FLAG (spec conflict, needs Xero): token spawn position - code says TOP-CENTER, memory/lessons say TOP-LEFT (1,1).** Found 2026-05-22 while mapping L2 for Phase 2. `lib/tactical-spawn.ts:defaultSpawnCell` returns `{ grid_x: floor(gridCols/2), grid_y: min(1, gridRows-1) }` = top-CENTER, and its comment documents a deliberate "v3" evolution (v1 top-left was hidden by the fog toolbar, v2 top-right by the NPCs sidebar, v3 top-center is the only never-covered column). BUT memory `feedback_token_spawn` + lessons.md Tactical Map both say "tokens always spawn at top-left (1,1), never top-right ... top-left is the final decision." These conflict. I did NOT write a test (it would cement whichever is wrong). Resolve: is top-center (current code, with its furniture-avoidance rationale) correct, or should it revert to top-left (1,1)? Then update the loser (code or the memory/lessons note) and add the test. Low urgency; surfaced by the audit.
