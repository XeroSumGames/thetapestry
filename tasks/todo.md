...
- [ ] **[OP+PF] #1 BLOCKER - signup works end-to-end on prod.** `verify-turnstile` 503s today (Upstash env missing, route.ts:108-114) + needs `TURNSTILE_SECRET_KEY`. [OP] set Upstash KV + Turnstile secret in Vercel; [PF/manual] real prod signup smoke + invite-code (`signup_codes`) mint+redeem (E2E cannot automate Turnstile).
  ...
  - [ ] **[ROUTED -> HUNT & PECK, URGENT beta-blocker] remove invalid Turnstile `size: 'invisible'` render param.** Real signups failed "bot check failed"; Sentry `TurnstileError` (release 13854c47c6e2, /signup): *"[Cloudflare Turnstile] Invalid value for parameter \"size\", expected \"compact\", \"flexible\", or \"normal\", got \"invisible\"."* Turnstile has NO invisible `size` (that's a reCAPTCHA-ism); invisibility = the WIDGET MODE in the Cloudflare dashboard. **FIX:** delete the `size: 'invisible',` line in BOTH `app/signup/page.tsx:69` AND `app/login/page.tsx:59` (the widget is already positioned off-screen + auto-solves via the `callback`), then redeploy. **[OP] also set the Cloudflare widget Mode -> Invisible** to match the off-screen auto-solve design (Managed mode can pop an interactive challenge that hangs off-screen). After deploy: re-run the incognito signup smoke.
- [ ] **[PF+HP] #2 BLOCKER - `characters` cross-user write data-loss CLASS (trade is just flow 1).** Sibling-RLS audit DONE 2026-05-24 + verified: `characters` UPDATE is owner-only + Thriver bypass; **a GM is not a Thriver**, so the GM loot/award/ration loop (8 flows) silently loses data at a beta with non-Thriver GMs. Combat is SAFE (HP/conditions are on `character_states`, which has a member/GM policy). Finding: `tasks/finding-characters-rls-cross-user-writes-2026-05-24.md` (Risk Register RED).
  ...
  - **PC-PC trade (flow 1): DECISION = Option B, the RPC (Xero 2026-05-24, "we'll need it eventually, don't cut corners").**
    ...
    - [ ] **[ROUTED -> HUNT & PECK] (SOLE remaining trade blocker) rewire `onGiveItem`** (`app/stories/[id]/table/page.tsx` ~6925) to call the RPC instead of the raw `.from('characters').update(...)`, and REMOVE the giver-side decrement too (the RPC does both - else double-spend). Keep `notify_inventory_received` + the `inventory_transfer` broadcast; refetch/patch both characters' local inv; surface RPC errors. Full contract in the SQL file footer. (App code = HP lane; PF does not edit it.)
    - [ ] **[ROUTED -> E2E] un-fixme the PC-trade assertion** once the RPC + client land (the trade test is currently `test.fixme`).
...
- [ ] **Footgun (defensive, not yet hit in prod):** `placeTokenOnMap` + ~6 sibling table-page functions read the active scene with `.single()`, which hard-errors if 2+ scenes are ever active (the racy New Map could produce that) -> "No active tactical scene" alert even when scenes exist. Harden to `.order(...).limit(1).maybeSingle()`. Lower priority now that the double-fire guard reduces multi-active risk.
...
- [ ] **Footgun audit (from the Scene Name fix):** find other free-text `<input>`/`<textarea>` bound directly to realtime-synced state with write-per-keystroke - same "echo clobbers typing" class. Candidates: GM notes / vehicle notes / character notes / campaign fields. Apply the local-draft + commit-on-blur pattern where found.
...
- [ ] **[ROUTED -> HUNT & PECK; puffer triage 2026-05-26] Player tile (bottom bar): (1) the name must NEVER vanish; (2) the state-chips grid goes to the RIGHT of POPOUT, not on its own row below.** Location `app/stories/[id]/table/page.tsx` ~7728-7741. (1) NAME VANISH: the name div (L7735) is `whiteSpace:nowrap; overflow:hidden; textOverflow:ellipsis; maxWidth:100%` - on a narrow tile (the MAP/avatar/POPOUT flex row is the only width driver) it truncates to nothing. Fix so it always shows (allow 2-line wrap, or min-width the tile, or drop the hard clip) - Xero: "should never vanish". (2) STATE CHIPS: `<PlayerStatusChips>` (L7741) currently renders on its OWN row below the name - move it INTO the flex row, to the RIGHT of the Popout button (~after L7733). **This REVERSES the 2026-05-25 decision (`296b7f2` + the L7738-7740 comment "state chips on their own row ... keep that row symmetric") - Xero now wants them right-of-Popout; update/remove that comment.** Watch: chips back in the row will re-introduce the row-width asymmetry the 5/25 change avoided - right-align or let the row grow; Xero has accepted that tradeoff. Pure HP (table-page tile + maybe PlayerStatusChips); no Puffer/DB work.
...
- [ ] **[ROUTED -> HUNT & PECK] roll_log feed MISS: "Minnie's damage roll didn't appear in the logs" (00-15 #1).** This is the roll_log feed item flagged YELLOW/held-for-this-playtest materializing. "Minnie" = the campaign vehicle (RV), so likely a VEHICLE damage roll (or a roll routed through the vehicle/Minnie) never produced a roll_log row. HP: check the damage-roll write paths (the 2026-05-19 set: Advantages/single-modal FI/Stress-12/Stabilize cascade, + vehicle damage) for one that skips `insertRollLog`. Concrete repro target: a damage roll involving Minnie didn't log.
...
- [ ] **[PUFFER - pin realtime propagation; TWO corroborating reports].** (00-15 #2) "i shared a PIN and it didn't show up withoiut a refresh" + (02-22 #4) "pin didnt' show to players as they neared it". Both = a pin that SHOULD be visible to players didn't appear without a manual refresh. FINDINGS (puffer, verified in code): campaign pins are `campaign_pins` (the Leaflet map; `map_pins` = tactical); BOTH are in `sql/_baseline/publication.sql` (L25, L33). Campaign pins do NOT rely on postgres_changes (code says it was "laggy") - they propagate via a **broadcast `pins_changed`**: sent in `components/CampaignPins.tsx:163` on a pin change, received at `CampaignPins.tsx:169` + `CampaignMap.tsx:913` -> `loadPins()`. So the break is in the reveal->broadcast->reload->RLS chain. **PUFFER INVESTIGATION DONE 2026-05-26 - root cause found, fix is HP's lane:**
  ...
  - **[ROUTED -> HUNT & PECK] FIX (components/ = their lane):** add CATCH-UP RELOADS so convergence doesn't depend on catching one ephemeral broadcast - call `loadPins()` on the channel's `SUBSCRIBED` (re)connect status AND on `document` `visibilitychange`/focus regain, in BOTH `CampaignMap` and `CampaignPins`. Low-risk; covers the bounce / late-subscribe / dropped-packet cases. **SWEEP RESULT (Puffer 2026-05-26): the catch-up pattern ALREADY EXISTS in the app - `RollsFeed.tsx:246-249` and `TableChat.tsx:165-168` both `refetch()` on `visibilitychange`; `table/page.tsx:1388/1447` does SUBSCRIBED + visibility. The PINS subs are the OUTLIER that missed it (`CampaignPins.tsx:166-176` + `CampaignMap.tsx:912-915` reload ONLY on the broadcast). So HP's fix = COPY the existing `RollsFeed` pattern onto the two pin subs - not a new mechanism. SECONDARY AUDIT (same potential gap, lower stakes): the other 3 broadcast-only subs - `PlayerNotes.tsx`, `app/npc-sheet/page.tsx`, `app/campaign-sheet/page.tsx` - check each for a visibility/SUBSCRIBED catch-up; add the same pattern where missing.**
...
- [ ] **[ROUTED -> HUNT & PECK] "Share route" with players (GM note #7).** Verbatim: "how to \"share route\" with players". The CampaignMap Route planner (`components/CampaignMap.tsx:170+`) computes a route but the GM found no way to share it with players (he set waypoints, then hit Stop). Feature/UX gap - add a share-route affordance mirroring the existing Share View / Share Map. HP (campaign map).
...
- [ ] **Phase 1 remaining (1 BLOCKED):** only messages-dm left (parked on Xero's teardown call below).
...
- [ ] **Phase 1 BLOCKED - messages-dm:** DMs are append-only at RLS (NO DELETE policy on `messages`/`conversations`/`conversation_participants` anywhere in `sql/`), so a test message can't be torn down via the user-token pattern. Needs Xero's call: (a) read-only test only (page renders), (b) accept one `[E2E]`-tagged residue per run in the GM<->marv test thread, (c) provision a service-role teardown key (test-suite security-posture change), or (d) skip. Flagged 2026-05-24.
...
- [ ] **TEST DEBT - `section-c-npc-reveal` now fails DETERMINISTICALLY (3/3), needs a rewrite (flagged 2026-05-25 pre-playtest).** The DELTA-against-Arena approach degraded: the lowest-id Arena NPC's name now recurs ~51x in other NPCs' relationship lists, so the reveal's +1 roster card is lost in the noise. Tried a unique-name seeded NPC fixture -> ALSO fails (live=0 reload=0): the player's NPC panel renders `revealedNpcs`, which is built from `npc_relationships` (table/page.tsx ~7253), so a BARE NPC with no relationships never shows for a player regardless of `hidden_from_players` - the seeded fixture was invalid. NOT a confirmed reveal regression (DB write succeeds; realtime CORE - a1/a3/presence - passes; it was GREEN ~40 min before failing). REWRITE: seed an NPC WITH a relationship (so it's eligible for the player panel) in a throwaway campaign with marv as member, or assert on `revealedNpcs`'s actual driver. Until then it's a known deterministic failure (NOT caused by the PC-trade change). Manual 2-client reveal check confirms the FEATURE works for the 2026-05-25 playtest.
...
- [ ] **WATCH - realtime-delivery flakes under full-run load (retries absorb):** `section-c-npc-reveal` (reveal-count), `section-d` D-3 (UI community-create raciness), and now `section-e-pins` (approved-pin player refetch, flaked in the 2026-05-24 117-pass re-cert, passed on retry). All three are cross-context realtime-timing transients, NOT product regressions (they pass on retry; a real break fails every attempt). If any starts failing ALL attempts, treat as a real regression. No action yet - retries:2 is the intended absorber.
