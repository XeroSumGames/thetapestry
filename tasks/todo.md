...
- [ ] **[ROUTED -> PLAYWRIGHT/E2E] build combat-flow.spec.ts Phases A + C.** Phase A (Start Combat -> `initiative_order` rows + player IN-COMBAT + cascade-delete teardown) was unblocked already; Phase C (deterministic damage propagation, mortal-wound auto-fill, end-of-combat infection modal) now unblocked by the RPC above. Plan: [tasks/e2e-combat-flow-plan-2026-05-30.md](e2e-combat-flow-plan-2026-05-30.md). Phase B (initiative-bar DOM ordering / action decrement) still waits on the 4 HP testids (separate ask once HP wraps move-follow).
  - [x] **Phase A SHIPPED 2026-05-30** - `e2e/combat-flow.spec.ts` test 1: throwaway campaign + marv joins by code + GM seeds NPC + Start Combat -> asserts `initiative_order` exactly one is_active=true + `character_states` per PC combatant + player IN-COMBAT live + cascade-delete teardown. Standalone green 11s. Banner refreshes on the next full re-cert.
