...
- [~] **Y12 - Backup / restore playbook.** PARTIAL 2026-05-19. [tasks/ops-backup-playbook-2026-05-19.md](ops-backup-playbook-2026-05-19.md) documents: Supabase tier-by-tier coverage (free = no PITR, Pro = +$100/mo add-on for 7-day PITR), in-app campaign-snapshot recovery (covers + limits), 5 recovery scenarios with concrete runbooks (single row, full campaign, db-wide, GDPR delete, storage object), and the drill plan (~2h end-to-end). Still owed: the actual drill against a real Pro project. Drill plan is in the doc; ship the drill before paid signups open.
...
- [ ] **Brew Check modal rework** (flagged 2026-05-19 by Xero). The vehicle Brew check today goes through the generic `pendingRoll` modal with label dispatch in the post-roll handler (`brewMatch` regex at `lib/roll-helpers.ts:641`). Needs a redesign pass - exact scope TBD when Xero is ready to spec it. Possible directions: dedicated `<RollModal>` instance with brewer/vehicle/skill picker + outcome cascade (success → fuel produced, failure → botched brew narrative + maybe wasted resources), OR keep modal generic but add proper output presentation (yield amount, time cost, side effects).
...
- [ ] **Modal unification (Stabilize / Distract / First Impression remaining)** - Coordinated Effort migrated (`6640b1a`). Group Check resolved as not-bespoke (current banner is canonical, spec-group-check.md). Gut Instinct unblocked + shipped today (`adb9382` GM whisper-detail modal). Phase 1 (Stabilize) spec at [tasks/spec-stabilize-migration.md](spec-stabilize-migration.md). Phase 2-3 chain behind.
