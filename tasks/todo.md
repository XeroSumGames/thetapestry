...
- [ ] **BUG (higher priority - stalls combat): combatant who dies from their own grenade stays in initiative + blocks turn advance.** Repro: NPC Hugo Vale threw a grenade into a cluster including himself, died from the self-blast, but stayed in the initiative bar and `nextTurn` couldn't advance. The active-combatant-dies-during-own-action path isn't handled. Lives in the blast-resolution branch of `executeRoll` + the death-removal / `nextTurn` handling (`app/stories/[id]/table/page.tsx`). Detect that the active combatant died from their own action's damage and proceed past them.
