...
- [ ] **[PUFFER][MEDIUM] AUDIT M1 2026-06-12 - sync `sql/_baseline/schema.sql` to live DB.** 6 drifts: `portrait_bank.is_private` + `.name` + nullable `number`/`gender` + drop `portrait_bank_gender_check` constraint + `initiative_order.pending_action_loss`. Live DB is correct (types regenerated); baseline is stale. Fix = text edits to baseline only, no DB action.
- [ ] **[PUFFER][MEDIUM] AUDIT M2 2026-06-12 - fix `loot_npc_item` em-dashes.** `check:db-emdashes` flags em/en-dashes in SQL comment lines inside the live function body. Fix: `CREATE OR REPLACE` from the baseline version (already clean) to sync `pg_proc` body with the no-em-dash rule.
