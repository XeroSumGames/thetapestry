...
- [ ] **BUG: grenade friendly-fire warning fires for NPC throwers hitting PCs.** Root cause `app/stories/[id]/table/page.tsx:7730-7732`: `friendlyCharacterIds = entries.map(e => e.character.id).filter(cid => cid !== activeEntry.character_id)` treats EVERY other combatant (PCs + NPCs) as a "friendly" regardless of faction. So a GM throwing an NPC grenade at the PCs gets warned "This blast will hit: Cree Hask (Engaged)" - but hitting a PC is the NPC's intent, not friendly fire. Consumed at `components/TacticalMap.tsx:2904-2927`. Fix options: (a) faction-aware friendlies (NPC thrower friendlies = same-disposition NPCs, not PCs); OR (b) suppress the warning when `activeEntry.npc_id` is set (GM sees the whole board). Xero intent "only fire for actual friendly fire" leans (b) or (a)-done-right. Pick with Xero.
- [ ] **BUG (higher priority - stalls combat): the ACTIVE combatant going down during their own turn does not auto-advance.** Two manifestations, one root cause: (1) NPC Hugo Vale threw a grenade into a cluster including himself, died, stayed active, blocked advance. (2) PC Cree Hask dropped to mortally-wounded (own-blast/cluster), remained the active combatant (-> CREE H.) on the player view, unable to act ("you can still watch the map... read the log"). **Root cause:** the skip-downed-combatant logic at `app/stories/[id]/table/page.tsx:2169-2210` only runs when `nextTurn` walks FORWARD to the next actor (skips PCs where `wp_current===0 || rp_current===0` at L2201, dead/0-WP NPCs at L2195). It does NOT handle the active combatant going down DURING their own turn - they keep `is_active=true` and combat stalls until manual intervention (and the manual NEXT can be obstructed by the death cascade). **Fix direction:** after damage resolution drops a combatant to MW/incap/dead, if they are the active combatant, auto-fire `nextTurn` (or clear their active flag so the GM's NEXT advances cleanly). Lives in the post-damage / blast-resolution branch of `executeRoll` + the `nextTurn` active-combatant handling. NOTE: mortally-wounded PCs should STAY in the initiative list (they can be stabilized) - just not be the active turn-taker. Only truly-dead (death_countdown expired) combatants should leave the rotation.

- [ ] **BUG: Coordinated Effort produces no persistent feed/log entry (only a transient active-chain banner).** ROOT-CAUSED via 3-browser playtest dumps 2026-05-21 (percy_bent = Frankie's player). The mechanics WORK: Frankie (PC, `is_npc: false` confirmed in trace) fired the lead roll (`closeRollModal gate {didRoll: true, wasWeaponAttack: false}`), action consumed (`2 -> 1, rowsAffected: 1, error: none`), chain activated (the "COORDINATED EFFORT ACTIVE - +1 coord +2 lead = +3 CMod" banner shows). **The bug is purely presentational:** that "+3 CMod" banner is a transient top-of-screen active-chain indicator, NOT a feed entry. The lead roll's `saveRollToLog` insert (`app/stories/[id]/table/page.tsx:4905`, stamps `coord_chain_id` per L4895-4903) writes a row, but `collapseCoordEffortChains` (`components/RollsFeed.tsx:122-181`) only renders the bespoke "Coordinated Effort" feed banner once PARTICIPANTS have rolled (L160-176 enrich path); a lead-only chain falls through L154-158 as a **plain dice row**, so the activation reads as nothing meaningful in the log. **Xero's ask ("this needs to move to the logs"):** the coordinated-effort ACTIVATION should render as a bespoke feed banner immediately on the lead roll (e.g. "Frankie starts a Coordinated Effort using <skill> - +N CMod to all participants"), persisted in the feed, mirroring the active-chain banner's content. **Fix:** in `collapseCoordEffortChains`, render a bespoke lead-only banner (don't pass the lead through as a plain row); RollsFeed already has the Coord-Effort Tier-A banner renderer at L776+ - extend it to the lead-only state. Found in the 2026-05-20 smoke, root-caused 2026-05-21.
...
- [ ] **Grenade/Molotov throw-time auto-decrement.** The carry-qty stepper is manual (set/adjust by hand). When a PC/NPC throws a grenade in combat (the explosive attack path in `executeRoll`), the equipped weapon's `qty` should auto-decrement by 1, and the slot should clear/disable when it hits 0. Separate touch in the table-page explosive attack flow. Follow-up to the carry-qty feature above.
