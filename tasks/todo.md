...
- [ ] **Phase A2** - rework `RollModal.tsx` to the LOCKED shell: 400->340, three-zone reserved-height middle, inline CMod (drop the full-width block), always-render Insight option. Update the 8 roll callers for the new shape. Pure-visual first.
- [ ] **Phase A3 (FUNCTIONAL)** - Gut Instinct sub-skill DROPDOWN (CONFIRMED Xero 2026-05-24, build it; player picks + defines narrative). Stress/Breaking Point/Lasting Wound get NO insight (Xero rethought 2026-05-24, rewriting canon - no insight wiring needed anywhere). Recruit pick-step treatment PENDING Xero's lose/gain call (the pick step is a rich config form - rolling-PC, target, community+new-group+naming, approach lock-gates, poaching penalty, skill, CMod - not a 3-dropdown roll; recommend keeping it a wider form with shell chrome, not clamped to 340).
...
- [ ] **Phase 1 remaining (4):** campfire-social (forum/LFG/war-story + Thriver auto-approve), presence ("Survivors present: N"), world-pin-to-queue (moderation), account-settings.
