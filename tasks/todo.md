...
- [~] **[PRE-EXTRACTED CANON for HP pickup 2026-05-31] Rest finish - 3 wired gaps + 1 verify** - canon walked top-down by Puffer in [tasks/canon-extract-rest-2026-05-31.md](canon-extract-rest-2026-05-31.md).
  ...
  - **C (post-mortal slow regen persists to wp_max) STILL OPEN - needs live DB change (Xero go-ahead).** Plan: additive `character_states.recovering_from_mortal_wound boolean default false` + a BEFORE-UPDATE trigger that sets it true when `wp_current` hits 0 and clears when `wp_current >= wp_max`; Rest modal reads the flag instead of current WP. SQL drafted for review (`sql/character-state-mortal-recovery-flag-2026-05-31.sql`). Bright-line live-DB change; apply after intent confirmation. Modal-redesign migration remains OUT of scope (Phase E).
