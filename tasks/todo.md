...
- [ ] **[MEDIUM] UX: infection has NO at-a-glance indicator.** Found 2026-05-24 (this is what made the infection bug look like a re-arch failure for an hour). `infection_state` is only surfaced by the CharacterCard "Infection" button's PROMPT when clicked ("Currently sick (wound, N days left)") - no chip, badge, or color on the card or player bar. A GM/player cannot tell a character is infected without clicking. Add a visible "infected" chip alongside the lasting-wound chips (CharacterCard + player bar), colored as a status condition, showing kind + days left. Mirrors the lasting-wound chip pattern.
