...
- [ ] **NPC ACU (Acuity) attribute schema gap** — `campaign_npcs` table has only 4 attribute columns (physicality, dexterity, reason, influence); ACU is missing. Effect: any ACU-based skill check on an NPC reads 0 for AMod regardless of how the NPC was statted. Affects: Navigation (vehicle navigator), Farming, Gambling, Lock-Picking. Fix path: ALTER TABLE campaign_npcs ADD COLUMN acuity smallint NOT NULL DEFAULT 0; then backfill the NpcCard / setting-npcs editor UI to expose it; then any code reading n.acuity gets the real value. Larger than it sounds because every NPC card / NPC seed in lib/setting-npcs.ts would need an ACU value. Not blocking today since playtest navigators are PCs.
