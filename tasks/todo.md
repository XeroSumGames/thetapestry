...
- [ ] **[PARALLEL GAP found 2026-05-31] Rapid Fire / Fire from Cover / Cover Fire are not gated on ammo OR Broken condition** - `app/stories/[id]/table/page.tsx` (~`:6024` Fire from Cover, `:6077` Rapid Fire, `:5876` Cover Fire). The established gating only covers the primary Attack button, so an EMPTY or BROKEN weapon can still Rapid Fire / Fire from Cover. Same `isBroken` + `outOfAmmo` checks already computed in scope; add them to these buttons' onClick guards + disabled + label. Bounded follow-up (~20 min); not the #3 spot-check's scope but the same canon. Lower frequency than primary Attack.
