...
**RE-ARCH PHASE 5 PROGRESS - 4 of 6 god-components done (2026-05-23).** Table page (Phase 3/3d) + arch locked (Phase 4) done. Phase 5 propagates the seams to the other 6, ascending difficulty: [x] `app/moderate/page.tsx` data seam (`b59d147`/`d3aa2ac`), [x] `components/MapView.tsx` data+realtime seams (`4b528f9` - 36 `.from` + 2 `.channel` -> 0, behind `lib/data/map.ts` + new `lib/realtime/usePostgresSubscription`), [x] `app/vehicle/page.tsx` data seam (`deed757`, 14 `.from` -> `lib/data/vehicle.ts`) + realtime seam (`91e85fc`, 6 channels -> usePostgresSubscription + useCampaignChannel + new `lib/realtime/broadcastOnce`; combat-adjacent, 2-client smoke owed at Phase 7), [x] `components/NpcRoster.tsx` data + realtime (`d92aba5`, 46 DB `.from` + 2 storage -> `lib/data/npc-roster.ts` reusing campaign-npcs.ts; 1 channel/2 postgres -> useCampaignChannel postgres[]). **NEXT in order:** `components/CampaignCommunity.tsx` (3159 LOC, ~56 `.from`, 1 ch/1 pg), then `components/TacticalMap.tsx` (4315, hardest - 2 ch / 7 bcast / 6 pg). Per-component loop + reference checklist in [tasks/architecture-target.md](architecture-target.md); seam-migration technique in lessons.md top entry (now incl. broadcastOnce + ephemeral-sender pattern). Smokes batch to Phase 7.
