...
- [ ] **[NEEDS XERO INTENT, then likely HP/design] Heal-over-time (GM note #6).** Verbatim: "Enya heals another 3 wp over the next 12 hours". Is this a requested MECHANIC (medical-kit/rest delayed heal over in-game hours) or just a GM tracking note? If a feature: where does it live (rest/downtime? a medical-kit use?). Xero to clarify scope before routing.
- [ ] **[PUFFER - pin realtime propagation; TWO corroborating reports].** (00-15 #2) "i shared a PIN and it didn't show up withoiut a refresh" + (02-22 #4) "pin didnt' show to players as they neared it". Both = a pin that SHOULD be visible to players didn't appear without a manual refresh. FINDINGS (puffer, verified in code): campaign pins are `campaign_pins` (the Leaflet map; `map_pins` = tactical); BOTH are in `sql/_baseline/publication.sql` (L25, L33). Campaign pins do NOT rely on postgres_changes (code says it was "laggy") - they propagate via a **broadcast `pins_changed`**: sent in `components/CampaignPins.tsx:163` on a pin change, received at `CampaignPins.tsx:169` + `CampaignMap.tsx:913` -> `loadPins()`. So the break is in the reveal->broadcast->reload->RLS chain. SUSPECTS to check (2-client repro): (a) does the GM "Share/Show pin" reveal action actually `send({event:'pins_changed'})`, or only write the DB? (b) the PROXIMITY auto-reveal path - does nearing a pin write the reveal + broadcast at all? (c) the player `loadPins()` query + `campaign_pins` RLS - can a player SELECT a just-revealed pin, and does the reveal flag gate the row? (d) are players actually subscribed to the pins broadcast channel (channel name match)? This is mine; needs a focused dig + a 2-client smoke. NOT a publication-drift bug (both tables present).
