...
- [ ] **[MEDIUM][ROUTED -> HP] AUDIT M5 - realtime-wrap invariant: REAL FINDING.** Puffer audit (2026-05-30) found ~14 direct `supabase.channel(...)` sites OUTSIDE `lib/realtime/*` that bypass the Sentry wrap (the "100% coverage, not opt-in" hook claim is true only for code using the hooks). Group A sheet syncs (4), Group B map/pins (5), Group C feed/chat/notes (4), Group D popout (1), Group E lib/campaign-clock + bell (3). Full finding + bypass sites + closure-path decision (Path A migrate-to-hooks vs Path B wrap-at-call-site stopgap) in [tasks/finding-realtime-wrap-bypass-2026-05-30.md](finding-realtime-wrap-bypass-2026-05-30.md). **Recommendation:** Path B sweep first (~70 min, closes Sentry coverage gap fast), then add the guardrail script, then Path A migration alongside other god-component decomposition. Don't add the guardrail BEFORE the sweep - it'd break pre-commit on every bypass site.
