...
- [ ] **BUG: Coordinated Effort fires but silently does nothing for NPC participants.** Repro: Frankie did two coordinated efforts; neither logged nor applied. **Strong hypothesis (hunt-and-peck confirm): Frankie is an NPC** (the Conscript from the recruit test) and `triggerCoordinatedEffort` at `app/stories/[id]/table/page.tsx:4446-4478` is PC-only: `participants = entries.filter(...)` (L4448) where `entries` is the PC roster (character_states + characters join) - NPCs aren't in it. So an NPC-only selection makes `participants.length === 0`, the early return at L4449 fires, no roll, no log, no effect. Also `myEntry = entries.find(e => e.userId === userId)` (L4450) + the "include your own character" gate assume a PC lead. **Fix options:** (a) support NPC participants/lead in coordinated effort (widen the participant source to include rosterNpcs); OR (b) if coord-effort is intentionally PC-only, the UI shouldn't offer it for NPCs AND a silent return should become a visible error ("Coordinated Effort requires PC participants"). Decide intent with Xero. Found in the 2026-05-20 grenade/combat smoke.
