...
- [ ] **Stabilize button should pick from ALL mortally-wounded characters in range, not just the first.** Current code at [:5781-5786](app/stories/[id]/table/page.tsx:5781) does `entries.find(e => …mortal)` and `campaignNpcs.find(...)` — only one PC + one NPC max. If multiple PCs are bleeding out simultaneously, the button shows only one. Replace `find` with `filter`, render a small target picker (radio list of mortally-wounded combatants within 20ft) when more than one is eligible, fall through to the existing single-button behavior when exactly one.
...
- [ ] **Hide NPCs from players when starting combat.** When Xero hit Start Combat the player tab showed every NPC in the campaign roster instead of just the ones the GM had revealed. Today the Start Combat NPC picker probably leaks the full `campaignNpcs` list to player clients via the realtime subscription. Need: a `hidden_from_players` flag on `campaign_npcs` (default true for newly added NPCs, false once revealed/placed/in combat), plus a player-side filter that excludes hidden ones. Also audit every callsite that loads `campaign_npcs` from a player context — table page initiative bar, NPC roster, scene tokens, asset panel — to confirm none are leaking. May tie into the existing `revealed_to_players` flag if one exists; check `lib/setting-npcs.ts` and the NpcRoster component first to see what's already there.

### Rules / mechanics gaps
- [ ] **Crossbow + bow should require a Reload / Ready Weapon between shots.** Per SRD these are single-shot weapons that must be re-loaded as a separate action. Today they probably fire at the same rate as a pistol. Check `lib/weapons.ts` (or wherever the weapon DB lives) for a `requires_reload_between_shots` flag (or analogous `clip: 1` setup). If the data already supports it, the Attack button needs to gate on ammo and the Reload action needs to apply. Likely shape: weapon definitions for Crossbow + Bow get `clip: 1`, attacking auto-decrements ammo to 0, the Attack button disables until Reload (or Ready Weapon for the bow nock+draw) consumes another action and resets `ammoCurrent` to 1.
- [ ] **Weapon DB SRD audit — confirm everything is there (Club is missing).** Xero noticed Club isn't in the weapon picker. Pull the SRD weapons list (probably in `docs/Rules/XSE SRD v1.1.17 (Small).pdf` — use `pdftotext` per CLAUDE.md), diff against `lib/weapons.ts` (or wherever the canonical weapon list lives), add anything missing. Club, Improvised, etc. are likely candidates. Each entry needs name, category (melee/ranged/explosive), damage, RP%, range, traits.

### UX / friction
- [ ] **MessagesBell + NotificationBell need a "Mark all as read" affordance.** NotificationBell already has a red `DELETE ALL` in its dropdown header (per Xero's screenshot 2026-04-28); both dropdowns should also offer a `MARK ALL READ` button alongside it. Today the only ways to clear unread state are (a) click each row individually or (b) DELETE ALL — destructive. A non-destructive "mark read" leaves the row visible but flips the unread styling off. Shape: add a button next to `DELETE ALL` in each bell's header (UPPERCASE Barlow Condensed, secondary color) that bulk-updates `notifications.read=true` (or whatever the messages-side equivalent is) for every row currently rendered in the dropdown. Mirror the optimistic-update pattern so the badge clears instantly without waiting for the realtime echo.
...
- [ ] **Drag-to-bottom-left is blocked by a popup.** When trying to drag a token to the bottom-left of the tactical map, a popup (notification? sidebar? popout?) sits over the drop target and intercepts the drag. Identify which element is the offender — likely the notifications panel pinned at `left: 10px` (per memory `feedback_notification_position.md` says "fixed left:10px, below bell; never change"), or a chat preview, or the player bar. If it's the notifications panel, accept that the position is locked and move the BLOCKED zone — e.g., add a `pointer-events: none` to it during an active drag (drag-start sets a class on the body, drag-end removes it). Don't change the notification position.
- [ ] **Map pinging is too clunky — make it easier.** Current pinging is press-and-hold ~600ms on an empty cell (per [PLAYTEST_TODO.md:92](tasks/PLAYTEST_TODO.md:92) #31). User says it still feels awkward. Options: (a) shorten the hold to ~300ms; (b) add a dedicated "Ping" button in the tactical toolbar that turns the next click into a ping; (c) two-finger tap on touchscreens; (d) Shift+click to ping. Probably (b) is the cleanest — explicit affordance, no gesture timing — but discuss before shipping.
...
- [ ] **Stale realtime subscriptions — table tab needs manual refresh after backgrounding.** Reported by player GrumpyBattersby mid-playtest (2026-04-27): "very slow refresh, has to manually refresh his screen." Suspected root cause: the table page subscribes to 12+ Supabase realtime channels (`app/stories/[id]/table/page.tsx:1140-1323` — reveals/community/table/members/rolls/chat/initiative/campaign/npcs/presence/etc.) but there is **no `visibilitychange` / focus listener anywhere in the codebase** (grep confirmed zero hits). When Chrome backgrounds the tab for ~5+ minutes the websocket can silently die; tab "looks" connected but no events arrive until full reload. Fix shape: single `useEffect` on the table page (and probably the campaign page) that listens for `document.visibilitychange`; when going from hidden→visible, re-runs the initial-state fetches (`loadRolls()`, `loadInitiative()`, `loadChat()`, `loadNpcs()`, `loadCommunityMembers()`) **and** rebuilds the channels via `supabase.removeChannel(ref.current); ref.current = supabase.channel(...).subscribe()`. Test plan: open table page, switch to another tab for 10 min, switch back — chat/rolls/tokens/initiative should sync without F5. Also worth adding a small "stale connection" indicator (gray dot → red dot if last presence heartbeat > 30s) so players can self-diagnose. (Parked 2026-04-27 mid-playtest; deferred to avoid shipping untested realtime plumbing changes during a live session.)
