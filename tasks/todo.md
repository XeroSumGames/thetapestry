...
- [ ] **[ROUTED -> HUNT & PECK 2026-05-31][HIGH] wall-segment door toggles silently no-op for players** - LoS desync between clients during play (Xero saw one PC open a door, adjacent PC couldn't see through it). Root cause: `components/TacticalMap.tsx:537` `scheduleWallsPersist` gates DB write on `isGM`. Player door-clicks update local mirror only; no DB write, no realtime fan-out, other clients still see door closed. Window-segment toggles have the same bug (same code path at `:2769-2787`). Fix: SECURITY DEFINER `toggle_wall_segment_door` RPC (mirror `gm_apply_damage` pattern) + route player click-handlers through it. Finding: [tasks/finding-wall-segment-doors-player-write-2026-05-31.md](finding-wall-segment-doors-player-write-2026-05-31.md). Xero called this out as a RECURRING bug class - capture lesson on landing.
