...
- [ ] **[ROUTED -> PUFFER FISH] extend `gm_apply_damage` with `p_infection_risk boolean DEFAULT false` (XERO GREENLIT 2026-05-30).** Preferred shape per the combat-flow plan: ADD a 5th optional arg to the existing RPC rather than spinning a new `gm_set_infection_flag` sibling - one new arg keeps the client integration simple, and the existing `roll_log` row picks up the flag in `damage_json.infection_risk` for the E2E to assert. Contract: when `p_infection_risk=true` AND target is `pc` AND the damage crosses to WP=0 (mortal wound), set `damage_json.infection_risk=true` on the inserted `roll_log` row alongside the existing `via='gm_apply'` + `target_kind` + before/after fields. Unblocks the end-of-combat infection-modal E2E assertion (last item in the combat-flow plan's Phase C). On apply: ping E2E + I'll extend `combat-flow.spec.ts` to cover it.
