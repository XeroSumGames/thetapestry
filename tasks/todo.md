...
- [ ] **[MEDIUM] Confirm end-of-combat infection-modal fix.** 3d moved `infection_check_request` onto the stable `useCampaignChannel` subscription; verify it now fires. Bisect: did the "...may have to deal with infection" WARNING rows show in the feed during the fight? + `sql/diag-wound-infection-2026-05-23.sql` + a 1-combat repro. (Cross-ref the open "Wound-infection check missed" item below.)
