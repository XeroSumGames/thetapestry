...
- [ ] **Phase 2 (4 left of 7):** ~~char-create methods~~ (#8) + ~~session lifecycle~~ (#9) + ~~rumors publish->clone~~ (#14) DONE; remaining: combat flow+propagation, NPC roster CRUD + recruit gates, communities lifecycle + weekly check, inventory/trade (+ ~5 small `data-testid`s - combat-flow is the heaviest + partly dice-gated/manual, do it last + coordinate its table-page testids with Hunt & Peck).
