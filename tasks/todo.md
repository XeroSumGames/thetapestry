...
- [ ] **Reword wound-infection success line.** `lib/roll-helpers.ts:191` (UNCHANGED by re-arch), `shrug` branch / `kind === 'Wound'`: "shrugged off the wound infection" -> "'s wound does not become infected" (renders "Tucker Ortega's wound does not become infected"). OPEN: match the parallel Sickness line `:192` ("does not become sick")? PREVIEW-SYNC: must update `tasks/roll-feed-log-preview.html` in the same commit (guardrail enforces). (Xero 2026-05-21.)
- [ ] **Clean up the GM loot-distribution line.** `app/stories/[id]/table/components/LootModal.tsx:111` (UNCHANGED) - `` `🎒 Loot: ${itemList} → ${names}` `` renders "🎒 Loot: → Cree Blaine" when itemList is empty. Reword (e.g. "🎒 Cree Blaine received <items>" - confirm wording) AND guard the blank/empty item-name case (`:108` map; zero-item guard `:88`; button disabled `:117`). (Xero 2026-05-21, screenshot.)

**TIER 2 - gameplay bugs (small-medium; correctness)**
- [ ] **[HIGH] Can't see through OPEN windows (fog / line-of-sight).** NEW (was held in backup). `components/TacticalMap.tsx`: documented intent `:59-62` (walls block vision; windows pass vision); LOS gate `:1351-1358` blocks a door/window only when `door_open === false`. PRIME SUSPECT: the toggle/default at `:2635` defaults a window's `door_open` to FALSE when undefined (`w.kind === 'window' ? false : true`), while comment `:2808` says "window = open by default (vision passes)" - so an untoggled "open" window reads as CLOSED -> blocks vision. Also check render `:1760`/`:1822` + the fog `visible`-set sweep. CONFIRM RULE with Xero: do windows ALWAYS pass vision, or only when open? (Xero 2026-05-21, screenshot.)
- [ ] **Infection only from cut/shot, not punches/blunt.** Live path now `app/stories/[id]/table/hooks/useRollResolution.ts:595` (PC) + `:711` (NPC) - `if (finalWP > 0) pendingWoundInfectionRef.current.add(...)`, no weapon gate. (page.tsx:4915/4941 still hold inline dupes - confirm dead before assuming the hook is the only path.) Fix: gate on a skin-breaking weapon property (add to `lib/weapons.ts`; bladed/piercing/firearm = yes, blunt/unarmed = no); at minimum exclude Unarmed. CONFIRM blunt-melee scope (Club/Bat) + reconcile with Infection canon (CRB p.114-115; `project_infection_canon`) - if a new ruling, add to canon. (Xero 2026-05-21, screenshot.)

**TIER 3 - standalone (medium; not table-coupled, low collision)**
- [ ] **Presence count misses /table users.** "Survivors present: N" uses the `global_presence` channel + `.track()` in `components/Sidebar.tsx:85`/`:100`, but `components/LayoutShell.tsx` hides Sidebar on full-width routes (`FULL_WIDTH_PATTERN:20` matches `/stories/[id]/table`; `hideSidebar:197`), so table users never `track()` and drop off the count. Fix: relocate the global `track()` to an always-mounted layer (LayoutShell or a presence provider); count + hover-list stay in Sidebar. NOTE: the re-arch added a SEPARATE per-table channel `presence_table_${id}` at `page.tsx:1353` - that is in-table presence, NOT the global count; do not conflate them. 2-client verify. (Xero 2026-05-21, screenshot.)

**TIER 4 - low priority (zero-code workaround exists)**
- [ ] **`/record` dumps -> `C:\TheTapestry\playtestlogs`.** `lib/playtest-recorder.ts:244` `downloadDump()` (sets `a.download = name` at `:254`) -> browser Downloads. CONSTRAINT: a web page cannot write to an arbitrary disk path; `a.download` only sets the filename. Real in-app fix = File System Access API `showDirectoryPicker()` + persist the handle in IndexedDB (Chromium-only, one-time grant). WORKAROUND NOW (no code): set the browser's default download folder to `C:\TheTapestry\playtestlogs`. (Xero 2026-05-21.)

**TIER 5 - large (needs a design / schema decision before any code)**
- [ ] **Recruit into a GROUP, not a forced Community** (canon conformance). Canon `tasks/tapestry-rules-canon.md:746`: PCs + recruited NPCs are a **Group**; it becomes a **Community** only at 13+ combined members (Morale Checks). Recruit pick-step `app/stories/[id]/table/page.tsx:9501` ("Pick target & approach" `:9534`) wrongly forces founding+naming a community at recruit #1; result shell (RollModal) `:9772`. Fix UI: recruit into a Group with no name/public/Morale apparatus until member 13. DATA MODEL: recruits land in `community_members` tied to a `communities` row -> a pre-13 Group needs a non-community home (**SCHEMA-TOUCHING = confirm-first**; coordinate with Communities flagship). Confirm the at-13 UX (auto-prompt to name?). (Xero 2026-05-21, screenshot.)
- [ ] **Fold Advantages tab into Notes + redesign Notes.** Tab strip `app/stories/[id]/table/page.tsx:6983` (Pins / NPCs / Assets / ⭐Advantages / Notes); Advantages panel content ~`:7530-7610`; Notes renders `GmNotes`/`PlayerNotes` (dynamic imports `:62-63`) at `:7515-7516`. (1) fold the ⭐Advantages panel into Notes + drop the standalone tab; (2) redesign Notes to be "more functional" - needs a design pass (clarify scope: inline editing? search? per-character notes? pinning?). UX/design item, not a quick edit. (Xero 2026-05-21.)

> Also held (separate WORK, not a capture): **Modal visual unification Phase A** - reskinned `RollModal.tsx` + `lib/use-drag-position.ts` + 3 CharacterCard callers, backed up at `.claude/modal-phaseA-backup/` + `C:\Users\tony_\tapestry-backups\`. Per its `RESUME.md`, the re-arch likely reshaped the modals, so reconcile (don't blind-apply) when picked up.
