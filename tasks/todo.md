...
- [ ] **3c-B3 - extract executeRoll -> `app/stories/[id]/table/hooks/useRollResolution.ts` (THE big one; FRESH WINDOW per Xero's "don't start deep in a window" rule).** executeRoll is `page.tsx:5050-~6850` (~1860 lines after B1/B2; verify range at start - it ENDS just before `spendInsightDie`). Behavior-preserving relocation behind a typed deps object (the doc's "single RollContext instead of ~40 closure deps"). FIRST ACTION: enumerate executeRoll's external-identifier surface (state/refs/setters/helper-fns/async-fns it reads) - that list IS the hook's deps interface. Do NOT also split pure-decision-from-side-effects in the same commit (would make a 2-client smoke failure non-localizable). FIX-DO-NOT-PRESERVE carried in: none new beyond 3c-A's (CMod itemization + NPC defense already shipped surgically in 3c-A and verified). After extraction: `node scripts/check-arch.mjs --save` (page drops ~1800 more), then Xero 2-client smoke. OPEN watch (verify, don't assume): GM-rolled NPC attack showing `+5 CMod` generic = consistent with a MANUAL GM field entry (autoNet=0 -> manual term labeled 'CMod'); NPC-attacker defense IS wired via the prefill (page.tsx:4804 computeAttackCmod) - only change the prefill if a repro shows a target WAS auto-selected yet defense did not itemize.
