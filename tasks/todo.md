...
- [ ] **Vehicle passenger auto-positioning** — when a character is assigned to a vehicle slot via popout (driver/navigator/shooter/passenger), snap their scene token onto a fixed cell offset relative to the vehicle's footprint. Spec: driver = top-left of footprint, navigator = adjacent to driver, shooter = top-center, others = back row. Needs a slot-to-offset map keyed by vehicle size/shape (Minnie is grid_w=1 grid_h=1 visually but renders ~3-cell radius via scale=7.8, so "top-left of footprint" needs a working definition - probably "top-left of the visual circle"). Also needs an "exit vehicle" action so passengers can dismount without a GM workaround. Followup to vehicle-passenger-MOVE-sync (shipped 2026-05-15 via helper extraction).
- [ ] **Lock passenger tokens from independent drag while assigned to vehicle slot** — token drag-permission gate at `components/TacticalMap.tsx:~2846` (`canDrag = isGM || ownPC || isControlledObject`) needs an extra clause: if `myCharacterId` is assigned to any `campaigns.vehicles[*].{driver|brewer|navigator|passenger_seats[].character_id}` or any `mounted_weapons[].shooter_character_id`, block drag for that PC token (but skills/attacks still pass — they don't route through mousedown). GM bypass stays. Pairs with the auto-positioning feature; without lock, passengers drift off their slot cells.
- [ ] **NPC full-attribute backfill for vehicle Navigate skill picker** — `app/vehicle/page.tsx` `load()` crew-pool builder only pulls `dexterity` and `reason` from `campaign_npcs` (L117-119 query, L154-167 mapper). The Navigate check shipped with a skill picker that recomputes AMod from the selected skill's canonical attribute (Navigation=ACU, Survival=PHY, etc.); for an NPC navigator rolling a non-DEX/non-RSN skill, the AMod silently falls back to 0. Fix: widen the `campaign_npcs` `.select(...)` to include every attribute column (physicality, acuity, influence, instinct, intuition - confirm names against the schema) and populate `attributes` on the NPC `CrewMember` with all of them. ~10 lines. Known issue per the shipping comment at L172-178 in c6c8ad1; logging here so it doesn't bite when an NPC actually rolls Navigate on a non-DEX/RSN skill.
