...
- [ ] **BUG: Coordinated Effort fires but silently does nothing (Frankie, a PC).** Repro: Frankie (CONFIRMED PC by Xero 2026-05-20) did two coordinated efforts; neither logged nor applied. **The earlier NPC-only theory is WRONG - Frankie is a PC, so `entries` contains them.** `triggerCoordinatedEffort` at `app/stories/[id]/table/page.tsx:4446-4478`. Remaining candidate causes (need repro detail to narrow): (1) participant set or skill not set -> early return L4447 (`coordEffortParticipants.size === 0 || !coordEffortSkill`), silent, no log; (2) the "you must include your own character" gate at L4450-4454 firing - `myEntry = entries.find(e => e.userId === userId)`; if a GM fires the coord-effort on behalf of a player's PC (GM's userId != Frankie's owner), myEntry is the GM's own PC or undefined, so the gate alerts/returns; (3) the roll DID fire but `collapseCoordEffortChains` in `useRollsFeed` swallowed the constituent rows without rendering the chain banner. **Repro detail needed:** who fired it (GM or Frankie's player?), was a skill + 2+ participants selected, any browser-console error, any network roll_log insert. Capture that on next repro to pick the cause. Found in the 2026-05-20 combat smoke.
