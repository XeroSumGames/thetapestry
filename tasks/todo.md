...
- [ ] **Stabilize / Distract Phase 4 - retire legacy executeRoll branches.** Once the 2026-05-25 playtest verifies both dedicated modal paths are clean, delete the unreachable `if (pendingRoll.label.includes('Stabilize '))` AND `if (pendingRoll.label.endsWith(' - Distract'))` blocks in `app/stories/[id]/table/page.tsx` (~75 lines combined). Mark in commit as "Phase 4 cleanup".
