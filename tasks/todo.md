...
- [ ] **[PF+HP] #2 BLOCKER - `characters` cross-user write data-loss CLASS (trade is just flow 1).** Sibling-RLS audit DONE 2026-05-24 + verified: `characters` UPDATE is owner-only + Thriver bypass; **a GM is not a Thriver**, so the GM loot/award/ration loop (8 flows) silently loses data at a beta with non-Thriver GMs. Combat is SAFE (HP/conditions are on `character_states`, which has a member/GM policy). Finding: `tasks/finding-characters-rls-cross-user-writes-2026-05-24.md` (Risk Register RED).
  - **[PF] fix WRITTEN, apply gated (Xero):** `sql/characters-gm-write-rls-2026-05-24.sql` - GM-of-campaign UPDATE policy on `characters` (scoped, mirrors `character_states`); resolves flows 2-8. Apply: `npx supabase db query --linked -f sql/characters-gm-write-rls-2026-05-24.sql`.
...
- [ ] **Phase 1 GATE - Xero 2-client combat smoke** on the deployed extraction: driving + brew + navigate unchanged, AND a mounted-weapon attack still rolls damage, applies WP/RP to the target NPC, and decrements the shooter's action (the load-bearing path). Phase 2 is blocked until this passes. Revert if wrong = `git revert edb2032`.
- [ ] **Phase 2** (blocked on the smoke) - add `install` + `gather` CheckKinds to the hook + an ALL-CREW roller dropdown in preRollExtras (install gets a Mechanic*/Tinkerer toggle like brew; gather rolls Scavenging) -> applyInstallOutcome / applyGatherOutcome -> updateVehicle. Rewire the `+ Install` + `+ Gather Materials` buttons from direct calls to openCheck('install') / openCheck('gather'), keeping their disabled guards; Uninstall stays direct. Rulings locked + tested in lib/vehicle-checks.ts (install Dire = drum lost + 1 tank wasted; gather Wild = +2 days).
...
- [ ] **Phase 1 remaining (1 BLOCKED):** only messages-dm left (parked on Xero's teardown call below). (LFG "I'm Interested" + war-story composer also still uncovered - moderation trigger is shared so the role-gate generalises, but those composers' UI flows are untested.)
...
- [ ] **Phase 2 (1 left of 7):** ~~char-create~~ (#8) + ~~session lifecycle~~ (#9) + ~~rumors publish->clone~~ (#14) + ~~NPC roster CRUD~~ (#11) + ~~inventory~~ (#13) + ~~communities lifecycle~~ (#12) DONE; remaining: combat flow+propagation (#10 - heaviest + partly dice-gated/manual, do it LAST + coordinate its table-page testids with Hunt & Peck). All 6 shipped needed ZERO app testids (REST/DOM via existing title= attrs + the Community ▾ menu).
