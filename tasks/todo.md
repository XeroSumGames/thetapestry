...
- [ ] **[ROUTED -> HUNT & PECK 2026-06-12][MEDIUM] Advantage "Use" button should open the matching roll modal** - Currently `handleUseAdvantage` at `app/stories/[id]/table/page.tsx:1477` immediately consumes the advantage + logs to feed but opens no roll modal. Desired: clicking "Use" on e.g. a "+1 Melee Combat advantage" should open the roll modal pre-filled with "Melee Combat" as the label and `cmod_delta` as the CMod bonus; actual DB consumption deferred until the roll executes so cancelling the modal does NOT silently burn the advantage. Implementation: (A) add `pendingAdvantage: Advantage | null` state to table page; (B) in `handleUseAdvantage` replace immediate `consumeAdvantage` call with `handleRollRequest(a.skill_name, 0, 0)` + `setPendingAdvantage(a)` + seed CMod to `a.cmod_delta`; (C) in `executeRoll` after `insertRollLog` resolves: call `consumeAdvantage(supabase, pendingAdvantage.id, newRollLogId)` to link `consumed_roll_log_id` + `setPendingAdvantage(null)`; (D) in `closeRollModal`: also `setPendingAdvantage(null)` (cancel path - advantage NOT consumed). `AdvantagesPanel.tsx` `onUse` prop signature unchanged. Trigger: Xero screenshot 2026-06-12.
...
- [x] **[PUFFER] AUDIT M2 DONE 2026-05-30 (`52fe1d4`)** - TacticalMap 14-day watch note added to debug-handoff.md §1 covering the post-gate polish wave (`aea76cd`/`38e59cb`+`5845bfd`/`0599207`+`c3e0f10`/`421a4d6` etc) through 2026-06-13. NOT a YELLOW re-bump; if 2026-06-13 passes clean, drop the watch line.
