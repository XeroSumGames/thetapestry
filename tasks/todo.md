...
### 🧬 PREGEN SELF-SERVE LIBRARY (2026-06-18, Puffer design + SQL done)
- [~] **"PREGEN" button + user-generated pregen library.** Lets GMs/Thrivers publish any character they build as a reusable pregen (between Print/Save in the builder). Thriver -> auto-approved to global; GM (owns >=1 campaign) -> moderation queue + shows on their /characters with a status chip; approved -> global `/pregen`. Replaces hand-transcribing old pregen PDFs. Decisions locked 2026-06-18 (GM/Thriver-only; My Survivor = /characters; auto-tag setting). Spec: [tasks/spec-pregen-button-library.md](spec-pregen-button-library.md).
  - **[PUFFER DONE]** `sql/pregen-library-2026-06-18.sql` - table + indexes + RLS (mirrors modules moderation shape; RLS hardening: non-Thriver insert needs a campaign + can only write `pending`, no self-approve). **NOT yet applied live** - awaiting Xero go (bright-line schema change).
  - **[ROUTED -> HUNT & PECK]** UI build checklist in the spec: PREGEN button (role-gated) + `lib/data/` insert helper + /characters status chips + moderate-page Pregens section + new `/pregen` route (mockup `tasks/pregen-page-mockup-v1.html`) + story-page "for this story" source.
  - Canon audit of the old Chased pregen sheets vs current schema: [tasks/chased-pregen-canon-audit.md](chased-pregen-canon-audit.md). David Battersby already exists + is canon-correct in `lib/setting-npcs.ts`.
