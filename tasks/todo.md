...
- [ ] **[ROUTED -> HUNT & PECK 2026-05-31] hidden NPCs in initiative (GM-only, no-spoiler combat prep)** - `initiative_order.hidden_from_players` boolean column (SQL committed at `sql/initiative-hidden-from-players-2026-05-31.sql` - HP applies in same commit as UI); Add-NPC flow gets a "Hidden from players" toggle; `components/InitiativeBar.tsx` filters for non-GM viewers + shows HIDDEN chip on GM side; active-turn shows "Waiting…" placeholder on player tabs; 👁 Reveal button updates row + token `is_visible` + posts a reveal roll_log line. Couples to existing `scene_tokens.is_visible`. Finding: [tasks/finding-hidden-npc-in-initiative-2026-05-31.md](finding-hidden-npc-in-initiative-2026-05-31.md). **Trigger:** 2026-05-31 playtest, Xero manually moved NPC Dylan offscreen to hide the ambush from the kids.
...
- [ ] **[ROUTED -> HUNT & PECK 2026-05-31] grapple modal still on the old (pre-shell) inline JSX path** - `app/stories/[id]/table/page.tsx:8559+` (triggers at `:5709`/`:5995`, state at `:705+`, result at `:8690`). Migrate to `RollModal` per the locked spec; spec doc `tasks/modal-redesign-spec-2026-05-24.md`, finding [tasks/finding-grapple-old-modal-2026-05-31.md](finding-grapple-old-modal-2026-05-31.md). Polish, not a blocker (combat still works; visual + brand inconsistency vs every other roll modal). Discovered mid-playtest 2026-05-31.
- [ ] **[ROUTED -> HUNT & PECK 2026-05-31] move Record button to sidebar chrome (post-playtest lesson)** - Phase A: chrome ⏺ button in `components/Sidebar.tsx:185` icon row, Thriver-gated, user-scoped localStorage key; new `components/RecorderToggleButton.tsx` + `useChromeRecorderToggle` hook; bonus 1-line fix to Ctrl+Shift+M so it WARNS when recorder is off instead of silently dropping marks. Keep the `/table` cascade button untouched (Phase B unifies later). Finding: [tasks/finding-recorder-chrome-button-2026-05-31.md](finding-recorder-chrome-button-2026-05-31.md). **Trigger:** 2026-05-31 playtest, GM hit Ctrl+Shift+M 4-5 times to mark moments; ZERO captured because the recorder was never armed on the GM tab (Record button only exists on /table; GM was on /dashboard for the session).
