...
### 🚨 E2E regressions found 2026-06-11 (ROUTED -> HP)
- [ ] **[ROUTED -> HUNT & PECK 2026-06-11][HIGH] Clone NPC uses original name after rename** - `npc-roster-crud.spec.ts` PARKED. GM creates NPC "Roster One", renames to "Roster Edited" (persists in DB), then clones: clone is named "Roster One #2" not "Roster Edited #2". The Clone NPC handler in `NpcRoster.tsx` reads a stale name - likely a closure capturing the pre-edit value. Route: HP audit `NpcRoster.tsx` Clone button handler - ensure it reads name from live state/DB row not the initial render prop. Un-park spec when fixed.
- [ ] **[ROUTED -> HUNT & PECK 2026-06-11][MEDIUM] pins-catchup visibilitychange no longer triggers CampaignPins reload** - `pins-catchup.spec.ts` PARKED. After `pl.evaluate(() => document.dispatchEvent(new Event('visibilitychange')))`, the "Uncategorized" folder never appears (pin inserted via REST, no broadcast, so only catch-up can surface it). Likely caused by `29dc618` (feat(table): chat ping + 3x Chat-tab flash on incoming messages) changing which tab is active or intercepting the Pins tab click before the panel mounts. Route: HP check `CampaignPins.tsx` visibilitychange handler + tab activation flow after `29dc618`. Un-park spec when fixed.
...
- [ ] **[LOW][HP] AUDIT L1 - wound infection loop** at `useRollResolution.ts:1855` + mirror: confirm dedup is parallel-safe, then `Promise.all` it; otherwise comment why serial.
- [ ] **[LOW][HP] AUDIT L2 - empty `.catch(() => {})`** sweep across 5+ sites; categorize as best-effort / Sentry-worthy / should-bubble.
