...
- [ ] **[PRE-EXTRACTED CANON for HP pickup 2026-05-31] Rest finish - 3 wired gaps + 1 verify** - canon walked top-down by Puffer in [tasks/canon-extract-rest-2026-05-31.md](canon-extract-rest-2026-05-31.md). Gaps A (Stress Cooling Off track entirely missing - canon says -1 Stress per 8 uninterrupted hours doing something enjoyable; modal computes WP+RP only and never touches Stress), B (Sick state RP cap not enforced - canon says half-max ceiling during sickness; modal clamps to full rp_max), C (post-mortal slow regen `wasMortal` detection is wrong - flips off the moment wp_current goes >0; canon says 1 WP / 2 days persists until back at wp_max, needs a `recovering_from_mortal_wound` flag on `character_states`). Verify E (pending-heal +12h/+24h checkpoint handlers fire on Rest's `advanceClock` - probably already wired but spot-check before relying on it). Surface: `components/CharacterCard.tsx:1191-1273` Rest & Heal modal. Recommended ship: A+B+C one commit, E separate verify pass; modal-redesign migration is OUT of scope (Phase E territory). Precedence walked stopped at Tapestry canon - top of the stack was definitive on all four tracks.
