...
- **2026-05-15 Pin folders Phase 1** (`<pending>`) — per-user pin folders on the `/table` PINS tab + campaign hub. New `pin_folders` + `pin_folder_assignments` tables with strict per-user RLS (no GM/Thriver bypass — folders are private). Folder rail above the pin list (All / Unfiled / +New, with Rename + Delete on the active folder), per-pin `<select>` to file pins into folders. Phase 2 deferred: drag-drop, folder reorder, GM bulk-reveal-and-auto-file, cross-tab realtime. See `tasks/pin-folders-2026-05-15-testplan.md`.
- **2026-05-15 Pin category consolidation** (`b10976b`) — picker dropped from 9x2+dangler to clean 8x2 (16 categories). Dropped military/hospital/settlement/landmark + migrated 31 live pins via `sql/pin-category-consolidation-2026-05-15.sql`. Label renames: Business → Commercial, Distemper Timeline → World Pin. Resource icon 🎒 → 📦.
