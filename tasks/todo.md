...
- [ ] **BUG: Aim's +2 CMod shows in the roll modal but is NOT applied to the attack total or the feed log.** Found by Xero, 2026-05-22 (decomposition smoke Part 1, Aim step). Repro: combatant clicks Aim (+2 CMod), then attacks. The roll modal's CMod field pre-fills +2 (`page.tsx:4633-4652` reads `activeEntry.aim_bonus` and `setCmod(baseCmod...)`), BUT the resolved roll renders `[3+5] +1 = 9` (only +1 AMod) and the feed log reads `[3+5] +1 AMod = 9` - the +2 CMod is dropped from both the computed total and the breakdown. So Aim is currently a **no-op on the actual roll** despite displaying the bonus. Likely root cause: a disconnect between the displayed `cmod` state and the value `executeRoll` actually sums at roll time (stale-closure / the roll reading cmod=0), i.e. exactly the bug class the re-arch's `useRollResolution` (single `RollContext` object instead of ~40 closure deps) is designed to eliminate. **DISPOSITION: fix during the Phase 3 roll-engine rebuild (`useRollResolution`), NOT now** - patching the 12.5k-line `executeRoll` is throwaway work the rebuild supersedes. **CRITICAL FLAG: the re-arch is behavior-preserving by default, so this bug MUST be explicitly listed as "fix, do NOT preserve" in the Phase 3 work or it will survive the migration.** Cross-ref: [tasks/grand-rearchitecture-2026-05-22.md](grand-rearchitecture-2026-05-22.md) Phase 3, and the Aim step in [tasks/decomposition-2client-smoke-testplan.md](decomposition-2client-smoke-testplan.md).
