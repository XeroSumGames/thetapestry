...
- [x] **A5.4 KV rate-limiter.** SHIPPED 2026-05-20 by hunt-and-peck lane. `app/api/auth/verify-turnstile/route.ts` now imports `@upstash/ratelimit` + `@upstash/redis`. Sliding-window 30/min/IP backed by Upstash KV (distributed across Vercel warm instances). In-memory bucket retained as dev fallback. Production-without-env-vars = 503 (loud misconfig signal).
...
- [x] **A5.3 RLS gap sweep.** SHIPPED 2026-05-20: [tasks/audit-rls-gap-sweep-2026-05-20.md](audit-rls-gap-sweep-2026-05-20.md). Potential-P0 SURFACED THEN CLEARED: 10 Tier-3 tables (`campaigns`, `campaign_members`, `campaign_notes`, `character_states`, `characters`, `map_pins`, `notifications`, `profiles`, `session_attachments`, `sessions`) had policies in `sql/` but no `ENABLE ROW LEVEL SECURITY` in repo. **Xero ran Query 1 2026-05-20: all 10 return `rls_enabled = true` in prod. NO P0, NO leak.** Remaining: reproducibility gap (add the ENABLE statements to repo so a fresh rebuild reproduces live state) - downgraded to documentation, folds into the canonical-DDL work. Tier-2 (16 tables, dashboard-only policies) extraction still queued for hunt-and-peck. 64 tables classified into 4 tiers; 23 fully covered, 16 RLS-on-but-policies-dashboard-only, 10 Tier-3 ambiguity, 4 no-coverage-at-all. 4-phase remediation RL1-RL4. SQL queries to dump dashboard state included for Tier-2 extraction.
- [x] **A5.5 Rate-limit coverage audit.** SHIPPED 2026-05-20: [tasks/audit-rate-limit-coverage-2026-05-20.md](audit-rate-limit-coverage-2026-05-20.md). Two scopes: A (server routes - 2 Next.js routes + 3 edge functions) + B (client-driven Supabase mutations across 64 tables). Scope A findings: `/api/health` + `log-visit` + `delete-user` all unlimited; `verify-turnstile` covered by L-3; `notify-thriver` effectively rate-limited via DB-trigger-only entry per Y2. Scope B: zero app-layer throttle; defense relies entirely on Supabase server-side limits. 5-phase migration plan RL1-RL5. Recommended: extract `lib/rate-limit.ts` shared helper from L-3 pattern, wire `/api/health` (120/min) + `log-visit` (60/min) + `delete-user` (10/min/user); defer Scope B until paid-signups OR abuse incident. **PHASE P4 COMPLETE** with this commit (A5.1 + A5.2 + A5.3 + A5.4 + A5.5 all closed).
...
**NEXT ACTION (puffer-fish lane):** **PHASE P4 (security hardening) COMPLETE + Xero soft-delete/invite rulings spec'd.** Added 2026-05-20: 3 SPEC-FIRST docs for the Xero rulings - [spec-character-states-preserve.md](spec-character-states-preserve.md) (Y11-a), [spec-roll-log-session-archive.md](spec-roll-log-session-archive.md) (Y11-e), [spec-invite-code-hybrid.md](spec-invite-code-hybrid.md). RLS Tier-3 verified clean (all 10 tables `rls_enabled=true`; no P0). Substrate is now dense with hunt-and-peck-executable inventory: 3 Y11 specs + DamagePayload (already shipping) + outcome-split + compactRollSummary + 7-item Y11/invite queue + decomposition Phase P1 + the P4 audit fix-ups (module-covers tighten, 5 bucket policies, 16 Tier-2 RLS extractions). **Strong recommend: real pause here for the hunt-and-peck lane to drain before more puffer-fish inventory adds noise.** Next puffer-fish move when ready: A5.5 rate-limit coverage audit (last P4 item) OR A2.1 realtime channel audit (P5). Neither is urgent; both wait for hunt-and-peck to catch up.
