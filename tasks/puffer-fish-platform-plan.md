...
**NEXT ACTION (puffer-fish lane):** write `tasks/audit-rate-limit-coverage.md` (P4/A5.5). Inventory of every API route + every authenticated mutation, mapped to whether a rate-limit applies. `/api/auth/verify-turnstile` has the in-memory limiter (with L-3 KV upgrade approved); `/api/health` is unlimited (low-risk read). Direct supabase-js writes from client-side have NO app-layer rate limit (Supabase enforces some). Audit surfaces gaps + recommends per-route action.
