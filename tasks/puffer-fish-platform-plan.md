...
**NEXT ACTION (puffer-fish lane):** write `tasks/audit-reentry-guards.md` - inventory + audit of the re-entry guard refs scattered across the table page (Phase P2/A2.4 of the platform plan). Today there are 5+ `*InFlightRef` / `Set`-based locks (`nextTurnInFlightRef`, `consumeActionInFlightRef`, `rollExecutedRef`, the consumeAction per-entry-id Set, etc.) preventing double-decrement / double-fire under React batching + realtime echoes. Each guard has its own scope + reset condition. The audit should: (a) enumerate every guard, (b) document scope + reset condition for each, (c) flag any guard that doesn't have an inline comment explaining it, (d) identify candidate consolidation patterns. Read-only spec - hunt-and-peck makes any actual changes after.
