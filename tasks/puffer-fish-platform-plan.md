...
- [x] **A5.3 RLS gap sweep.** SHIPPED 2026-05-20: [tasks/audit-rls-gap-sweep-2026-05-20.md](audit-rls-gap-sweep-2026-05-20.md). **POTENTIAL P0 SURFACED:** 10 tables (`campaign_members`, `campaign_notes`, `campaigns`, `character_states`, `characters`, `map_pins`, `notifications`, `profiles`, `session_attachments`, `sessions`) have policies in `sql/` but NO `ENABLE ROW LEVEL SECURITY` statement in repo. Either RLS is dashboard-enabled (likely, since app works) or NEVER enabled (every row leaks). Xero verifies via Query 1 in Section 3 - if any Tier-3 table is `rls_enabled = false`, it's a P0 fix. 64 tables classified into 4 tiers; 23 fully covered, 16 RLS-on-but-policies-dashboard-only, 10 Tier-3 ambiguity, 4 no-coverage-at-all. 4-phase remediation RL1-RL4. SQL queries to dump dashboard state included for Tier-2 extraction.
