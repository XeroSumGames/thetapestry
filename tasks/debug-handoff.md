...
### `map_pins` world-pin moderation - **RED (open security gap, MEDIUM severity - flagged 2026-05-24, fix designed, apply gated)**
- **What it is:** moderation of world-map pins (a non-Thriver's pin should go to a `pending` queue, not publish). Today the pending-vs-approved decision is made **only in the browser** (`MapView.tsx:948`, `QuickAddModal.tsx:236`); there is **no server-side enforcement** - no `BEFORE INSERT` trigger on `map_pins`, and the INSERT RLS only checks `user_id = auth.uid()` (+ suspension + rate limit), not `status`/`pin_type` by role.
- **What players (and the world) see if exploited:** a non-Thriver can skip the moderation queue with a direct REST insert (public anon key + their own session token), publishing a world-visible pin styled as a GM pin with zero review. Content-moderation evasion on a shared public surface. NOT a data breach / PII leak / privilege escalation beyond content.
- **Why RED:** it is an unmitigated, low-effort, currently-exploitable bypass on a public surface - same bug CLASS the campfire moderation trigger (`enforce_moderation_on_insert`, `sql/moderation-enforce-trigger-2026-05-17.sql`) already closed for `forum_threads`/`war_stories`/`lfg_posts`; `map_pins` was simply never included. Severity is MEDIUM (content-only), but an open server-unenforced security hole earns RED visibility until closed. Found by the E2E lane building `world-pin-to-queue.spec.ts`; full write-up + repro in `tasks/security-finding-map-pins-moderation-2026-05-24.md`.
- **Fix (designed, Puffer Fish; apply is a bright line = Xero sign-off):** a `BEFORE INSERT` trigger on `map_pins` (SECURITY DEFINER, reads `profiles.role`) mirroring the campfire enforce trigger - Thrivers' values respected; a non-Thriver's shared/world pin (`pin_type IN ('gm','rumor')` or `status='approved'`) forced to `pin_type='rumor'`, `status='pending'`; genuinely-private pins (`pin_type='private'`, owner-only via SELECT) left alone. Keys off `pin_type`/private-vs-shared (not `campaign_id` - world pins have none). SQL written + dry-run by Puffer Fish; applied via `npx supabase db query --linked` only on Xero's go; E2E adds the "Survivor REST insert -> forced pending" assertion after it lands. Tracked in `todo.md`.
- **First-place-to-look:** if a non-Thriver pin appears world-visible / styled as GM, this gap; the trigger (once it exists) + `map_pins` RLS in `sql/_baseline/schema.sql`.
...
- **TESTED (automated):** 532 unit tests across 29 files in `tests/lib/` covering roll-helpers (121), roll-outcomes (48), damage-payload (40), community-logic (29), table-roll-context (22), fuel-storage (21), safe-upload (20), brewing-supplies (19), first-impression-resolver (18), xse-engine (18), cdp-costs (17), damage (15), advantages (14), encumbrance (13), npc-drag-drop (12), range-profiles (12), distract-helpers (11), initiative-actions (11), sentry-filters (11), stabilize-helpers (10), gut-instinct-helpers (8), supabase-errors (8), tactical-view (8), playtest-recorder (6), rolls-feed-collapse (6), sentry-realtime (5), image-utils (3), signed (3), tactical-spawn (3). Suite runs in ~612ms on every commit + every push to main. (Auto-refreshed 2026-05-23 via `scripts/refresh-ledger.mjs`.)
