...
### `roll_log` writer path - **GREEN-ish (demoted 2026-06-12)**
...
- **Why demoted from YELLOW (2026-06-12):** YELLOW was held for "one more playtest cycle" after 2026-05-19 added new write paths (advantage_used, FI single-modal, Stabilize cascade, Stress 12-string). All of those except the Stress 12-string narrative fired cleanly across 3+ sessions including Session 63 (2026-06-12): Advantages grant->use->consume loop confirmed, FI modal feed row confirmed, Stabilize cascade 2x confirmed, Tier-2 Recruit feed rows confirmed. AUDIT M3 console sweep (`077f169`) replaced `console.error` at 97 sites with `reportSupabaseError` - no behavior change to feed writes. YELLOW condition met three times over.
- **Residual (STILL HOPED-FOR):** Stress Check 12-string narrative - 8 strings uncaptured: HEAL/UNJAM/REPAIR/Gut Instinct/Group Check/DRIVE/BREW/NAVIGATE. A bug there shows as a wrong/missing narrative string on the Stress feed row - visible but not session-breaking. Drain target: Beta-500 dry-run.
- **First-place-to-look:** if a feed row looks off, suspect the write path for that specific outcome type; check `lib/roll-helpers.ts:compactRollSummary` for the relevant branch.
...
### TacticalMap canvas - **GREEN (demoted 2026-05-30; 12-check 2-client gate ALL-PASS; 14-day watch CLEAN 2026-06-12)**
- **What it is:** `components/TacticalMap.tsx`. Renders the grid, tokens, fog, range circles, blast overlays.
- **Watch note CLOSED (2026-06-12):** The 14-day watch (2026-05-30 -> 2026-06-13) ran clean. Session 63 (2026-06-12) had active combat, token moves, 6 NPCs going mortal - zero tactical-map reports. Watch closed; watch line dropped.
...
- **Now:** 875 unit tests across 48 files in `tests/lib/` cover the high-value pure helpers and supporting modules (auto-refreshed 2026-06-12 via `scripts/refresh-ledger.mjs`):
  ...
  Suite runs in ~340ms on every commit + every push to main. The pre-commit hook gates on the full suite + 4 guardrails (font-sizes, role-literals, preview-sync, em-dashes).
...
- **TESTED (automated):** 875 unit tests across 48 files in `tests/lib/` covering roll-helpers (143), roll-outcomes (52), tactical-view (46), damage-payload (40), table-roll-context (40), upkeep (33), community-logic (29), env-damage (25), safe-upload (24), damage (21), fuel-storage (21), community-stage (20), brewing-supplies (19), rest (19), first-impression-resolver (18), xse-engine (18), cdp-costs (17), vehicle-checks (17), advantages (14), initiative-actions (14), tactical-grid (14), conditions (13), encumbrance (13), npc-drag-drop (12), range-profiles (12), vehicle-cover (12), vehicle-damage (12), distract-helpers (11), sentry-filters (11), tactical-spawn (11), campaign-route (10), combat-damage (10), stabilize-helpers (10), travel (10), combat-targeting (9), gut-instinct-helpers (8), login-redirect (8), supabase-errors (8), confirm-delete (7), inventory-cargo-load (7), playtest-recorder (6), rolls-feed-collapse (6), weapons (6), roll-log-stamp (5), sentry-realtime (5), image-utils (3), signed (3), toggle-lock (3). (Auto-refreshed 2026-06-12 via `scripts/refresh-ledger.mjs`.)
...
- **HOPED-FOR (shipped + typechecked but not played) - DRAIN PASS 2026-05-30 (post-Minnie-S7 + post-12-check-gate):** the 2026-05-19 batch (~50 commits post-2026-05-18 playtest). Drain status by item:
  ...
  - **DRAINED to PLAYTESTED RECENTLY (Arena Session 63, 2026-06-12 - Xero GM + Tony Bushell/Mikey Shevik + Percy Bent + Pesky Larue, ~38 min):** Tier-2 Recruit - Tony clicked Checks -> Recruit twice, selected NPC/skill/community, rolled "Roll Recruitment" twice. Both rolls fired cleanly; action consumed correctly. Community dropdown defaults to "pick a community" (HP finding filed). FI streamline (single-modal flow) - Tony did a First Impression roll at ~19:38:35; dedicated modal opened, roll fired, result rendered. Stabilize cascade (two attempts by Mikey Shevik on Frankie Gibblets at 20:05:10 and 20:05:30, each costing 1 action, actions 2->1->0 then nextTurn) - dedicated modal opened twice, both action-consume + cascade confirmed clean. Stabilize modal UI finding filed (duplicate "STABILIZE" header); no behavior regression. Core combat loop green: 6 NPCs went mortal (death_countdown:5), Charge/Grapple/Unarmed attacks + action consumption + nextTurn all fired without error across the full session. Note: Percy Bent and Pesky Larue had 0-event recorder dumps (recorder not turned on - HP finding filed).
  - **DRAINED to PLAYTESTED RECENTLY (Session 63 + post-63 live test 2026-06-12):** P3 Q4-b Advantages - GM grant confirmed Session 63; Use button now opens matching skill roll modal (`0f8e893`), confirmed live: Xero granted an advantage at 21:56:58, Tony clicked Use at 21:57:03, roll modal opened with target dropdown, Tony rolled + Done, `didRoll:true wasWeaponAttack:false cost:1` clean, nextTurn fired correctly. Full grant→use→consume loop verified. FI Insight Die AWARD path (rolling doubles) - the `insight_dice +1` increment at `useRollResolution.ts:264` has never fired in live play. Trigger: roll 1+1 (Low Insight, -3 NPC CMod + Insight Die) or 6+6 (High Insight, +2 NPC CMod + Insight Die) on any 2d6 check. 3 FI rolls in Session 63 missed doubles (2.8% each; ~36 rolls needed to expect one). Separately: the green-box SPEND side (Roll 3d6/Keep all 3, +3 CMod) IS working - Tony used "+3 CMod" in Session 63. What's untested is whether the award path correctly increments the pool the green boxes draw from. Stress Check 12-string narrative - Stabilize strings confirmed but HEAL / UNJAM / REPAIR / Gut Instinct / Group Check / DRIVE / BREW / NAVIGATE were not captured in Session 63 logs. Drain target: Beta-500 dry-run full pass (all 4 sections) before 2026-07-01.
  - **DRAINED to PLAYTESTED RECENTLY (Test Bed Session 24, 2026-06-16 - Xero GM + Tony Bushell/Knox Koss, ~95 min):** Recruit doubles -> Apprentice Eligible -> "Take as Apprentice" path confirmed functional (double-6 rolled, banner rendered, button executed cleanly - `page.tsx:10181` handler is clean: Supabase update + state + progression log + custom event, zero native dialogs). Advantage grant x3 by Xero (skill select -> note -> "Grant" cycle, all three fired cleanly). Combat core loop: 14 `npc_damaged` broadcasts received by Tony across two combat windows (00:54-01:05 UTC and 01:06-01:26 UTC); 8 NPCs went mortal with `death_countdown` set; incap path (`incap_rounds:3`) also confirmed on 2 NPCs before mortal. Campaign Map tile switching (Satellite/Topo/Street all three confirmed). Campaign Map location search ("Bonanza" -> real geocode results rendered). Share View one-shot push to Campaign Map confirmed working (Tony synchronized). Community Add Member flow confirmed (Tony added/deleted members, re-balanced roles). NPC rapid placement cascade (10 NPCs in <2s via Map button). **BUG FOUND + FIXED SAME SESSION (`550e252`):** Charge "Cancel Move" didn't clear `pendingChargeRef` (only cleared `moveMode`), causing the Charge state to persist invisibly - clicking Move again triggered the Charge attack roll modal unexpectedly, then after cancelling the roll, Move appeared yet again. Xero marked this twice. Fix: one-line reroute of the "Cancel Move" button to `handleMapMoveCancel()`. Testplan: [tasks/charge-cancel-testplan-2026-06-16.md](../tasks/charge-cancel-testplan-2026-06-16.md). One HP-routed finding: recorder mark `window.prompt()` at `components/PlaytestRecorder.tsx:244` surfaced as a jarring native dialog (Tony marked it as "browser level warning" - root cause diagnosed, fix routed to HP). **Still owed:** Stress Check 12-string narrative (HEAL/UNJAM/REPAIR/Gut Instinct/Group Check/DRIVE/BREW/NAVIGATE), FI Insight Die award path, vehicle popout broadcasts.
