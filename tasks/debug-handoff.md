...
- **HOPED-FOR (shipped + typechecked but not played) - DRAIN PASS 2026-05-30 (post-Minnie-S7 + post-12-check-gate):** the 2026-05-19 batch (~50 commits post-2026-05-18 playtest). Drain status by item:
  ...
  - **STILL HOPED-FOR (no fresh playtest evidence):** P3 Q4-b Advantages (schema + library + GM grant dialog + player tab + Use button + Award-on-feed + C3 broadcast) - unit-tested but not exercised at any multi-player table; NOT attempted in Session 63. FI Insight Die spend (green-box Low/High Insight paths) - single-modal confirmed working but Insight Die spend via the green boxes was not triggered in Session 63. Stress Check 12-string narrative - Stabilize strings confirmed but HEAL / UNJAM / REPAIR / Gut Instinct / Group Check / DRIVE / BREW / NAVIGATE were not captured in Session 63 logs. Drain target: Beta-500 dry-run full pass (all 4 sections) before 2026-07-01.
