...
### `characters` cross-user writes (GM loot/award + PC trade) - **YELLOW (GM half FIXED + verified live 2026-05-24; only PC-PC trade remains)**
- **UPDATE 2026-05-24:** GM-of-campaign UPDATE policy APPLIED + verified live (flows 2-8 persist). PC-PC trade (flow 1): Option B chosen + the `give_item_to_character` SECURITY DEFINER RPC APPLIED + verified live. **SOLE remaining gap: nothing calls the RPC yet** - the client `onGiveItem` still does the old raw `.update`, so PC-PC trade STILL loses data until Hunt & Peck rewires the client (`app/stories/[id]/table/page.tsx` ~6925) to call `give_item_to_character`. Stays YELLOW until that client rewire lands; then GREEN.
