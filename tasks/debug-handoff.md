...
- **UPDATE 2026-05-24:** the GM-of-campaign UPDATE policy (`sql/characters-gm-write-rls-2026-05-24.sql`) is APPLIED + verified live (policy "GM can update characters in their campaigns" present on `characters`). Flows 2-8 (all GM-driven loot/award/ration/narrative) now persist. **Remaining:** flow 1, PC-to-PC item trade (peer-to-peer, not covered by the GM policy) - decision pending: disable for the beta OR an inventory-only SECURITY DEFINER RPC [PF] + client rewire [HP]. Demote to GREEN once trade is resolved.
