...
### TacticalMap canvas - **YELLOW (render-model bug CONFIRMED at the 2026-05-26 playtest; fix spec'd + schema applied, HP implementing)**
- **What it is:** `components/TacticalMap.tsx`. Renders the grid, tokens, fog, range circles, blast overlays.
- **2026-05-26 (Minnie S7) - the live render bug (re-bumped to YELLOW):** the map was effectively UNPLAYABLE - players couldn't see their own tokens (rendered in the black void off the map edge), and the map rendered at a DIFFERENT scale per client ("not the same for all players"; "Juno on the edge but thinks she's in the middle"). ROOT CAUSE: `img_scale` was doing TWO jobs (the shared image-to-grid scale AND per-client viewport fit), was never persisted, and `1` was overloaded as both "unset" and "100%", so every client silently auto-fit the background to its own window width while the grid stayed at `cell_px` -> background slid under the grid by a different amount per screen. A locked map then stranded any mis-centered viewer (no pan). FIX: full spec `tasks/tactical-map-render-fix-spec-2026-05-26.md` (shared authoritative img_scale rendered for all + per-client fit via ZOOM, not silent rescale; locked-map "center on my token"). The unblocking schema is APPLIED live (`sql/tactical-scenes-scale-sentinel-2026-05-26.sql` - img_scale nullable sentinel + natural_w/h). **HP owns the client rewrite; demote to GREEN after the 2-client verify in the spec passes.**
